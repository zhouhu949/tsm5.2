...
import com.qftx.tsm.imp.dao.ImportResInfoMapper;
...
import com.qftx.tsm.imp.util.ImportVerUtil;
...
@Service
public class ImportResInfoService {
	@Autowired ImportResInfoMapper mapper;
	@Autowired ImportResFileService importResFileService;
	@Autowired ResCustInfoService resService;
	@Autowired ImportResResultService resultService;
	@Autowired ImportResResultDetailsService resResultDetailsService;
	...
	Logger logger =Logger.getLogger(ImportResInfoService.class);

	
	/**
	 *  查询 import_res_info
	 *  */
	public List<ImportResInfoBean> query(String orgId){
	 ...
	 *  */
	public List<ImportResInfoBean> queryByFileId(String orgId,String fileId){
		ImportResInfoBean entity= new ImportResInfoBean();
	...
	public void insertBatch(String orgId,String fileId,List<String> jsonDatas,Date date){
		try{
			List<ImportResInfoBean> beans = new ArrayList<ImportResInfoBean>();
			for (String jsonData : jsonDatas) {
				ImportResInfoBean bean = createBean(orgId, fileId, jsonData, date);
				beans.add(bean);
				if(beans != null && beans.size() == 500){
					mapper.insertBatch(beans);
					beans = new ArrayList<ImportResInfoBean>();
				}
			}
	...
	public ImportResInfoBean createBean(String orgId,String fileId,String jsonData,Date date){
		ImportResInfoBean bean = new ImportResInfoBean();
	...
	/* 插入*/
	public ImportResInfoBean insert(String orgId,String fileId,String jsonData,Date date){
		ImportResInfoBean bean = createBean(orgId, fileId, jsonData, date);
		mapper.insert(bean);
		return bean;
	}
	 ...
	 *  */
	public void setImpResult(ImportResResultBean bean ,List<ImportResResultDetailsBean> beans,String[] resultEnums,String orgId,String sysFileId,String resId){
		String rtnMsg = resultEnums[1];
		String resultEnum = resultEnums[0];
		ImportResResultDetailsBean detailBean = new ImportResResultDetailsBean();
	 ...
	 * 每500条提交一次
	 * */
	public void resultDetailInsertBatch(List<ImportResResultDetailsBean>irrdbs){
		if(irrdbs != null && irrdbs.size() >0){
			...
			for(int i=0;i<=count;i++){
				if(i<count){
					...
					resResultDetailsService.insertBatch(irrdbs.subList(i*500, (i+1)*500));
				}else if(yu>0){
					logger.debug("batchInsert:["+i*500+","+size+")");
					resResultDetailsService.insertBatch(irrdbs.subList(i*500, size));
				}
			}
		}
	}
	/** 查询 导入错误资源 */
	public List<ImportResInfoDto> getErrorResInfos(Map<String,Object>params){
		return mapper.findErrorResInfos(params);
	}
	
	/** 查询 导入错误资源 */
	public List<ImportResInfoBean> getErrorResInfos1(Map<String,Object>params){
		return mapper.findErrorResInfos1(params);
	}
	/** 
	 * 【忽略并导入】
	 * @param fileId 导入批次ID
	...
	public void ignoreImp(String orgId,String fileId,String errorCode,Integer state,String groupId,String account){
		try{
			...
			List<ImportResInfoBean> list =getErrorResInfos1(map);
			// 查询 资源导入_文件表 import_res_file
			ImportResFileBean importResFileBean = importResFileService.queryById(orgId, fileId);
			// 资源分组ID
			...
			for(ImportResInfoBean importTempResBean:list){
				...
				String resCustId = SysBaseModelUtil.getModelId();
					...
					if(ids !=null && ids.size()>0){	
						ImportResResultBean entity = new ImportResResultBean();
						entity.setOrgId(orgId);
						entity.setFileId(fileId);
						...
						if(entity != null){
							...
							// 删除掉导入错误信息
							resResultDetailsService.deleteByBatch(orgId,ids);
						}						
					}	
		}catch(Exception e){
