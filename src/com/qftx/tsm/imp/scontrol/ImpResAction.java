package com.qftx.tsm.imp.scontrol;
...
@Controller  
@RequestMapping("/resimp")
public class ImpResAction {
	Logger logger = Logger.getLogger(ImpResAction.class);
	@Autowired ImportResFileService importResFileService;
	@Autowired ImportResResultService resResultService;
	@Autowired ImportResInfoService resInfoService;
	...
	public String page(ModelMap model,String resGroupId,String status,String mark,String isDetail){
		...
		List<ResourceGroupBean> groups = importResFileService.findResGroup(user.getOrgId());
		// 销售进程缓存
	...
	public String resultPage(ModelMap model,ImportResResultDto dto){
		ShiroUser user = ShiroUtil.getUser();
		dto.setInputAcc(user.getAccount());
		List<ImportResResultDto> results = resResultService.query(user.getOrgId(),dto);
		if(results != null && results.size() >0){
			Map<String,String> nameMap = cachedService.getOrgUserNames(user.getOrgId());
			for(ImportResResultDto resultDto : results){
	...
	@ResponseBody
	public Map<String, Object> resultJson(HttpServletRequest request,ImportResResultDto dto){
    ...
    public void upload(HttpServletResponse response,MultipartHttpServletRequest request, String resGroupId,Integer status) throws Exception {
        ...
        if (file == null||file.getSize()==0) {// step-2 判断file
         	...
         	ImportResFileBean importFile = importResFileService.insert(user.getOrgId(),user.getGroupId(), resGroupId,status,user.getId(), sysFile.getId(), date);
         	// 获取excel 文件列头以及第一行数据
         	ExcelDataBean headerAndFistRowData = importResFileService.getExcelHeaders(user.getOrgId(), sysFile.getId(), filePath+fileName, date);
         	
         	// 修改 import_res_file 表 header，rowcount 字段
         	importResFileService.updateHeader(user.getOrgId(), importFile.getId(), JsonUtil.getJsonString(headerAndFistRowData.getHeader()),headerAndFistRowData.getRows());
         	// 获取 用户系统字段列表
         	Object[] obj = importResFileService.getImportFields(user.getIsState(),user.getOrgId());
	...
	public String ignoreImp(HttpServletRequest request,HttpServletResponse response){
		try{
			...
			resInfoService.ignoreImp(user.getOrgId(), fileId, errorCode, user.getIsState(),
    ...
    public String expErrorExcel(HttpServletRequest request,HttpServletResponse response){
    	try{
        	...
        	// 查询 匹配好的字段
        	ImportResFileBean importResFileBean = importResFileService.queryById(user.getOrgId(), fileId);
    		...
    		if(StringUtils.isNotBlank(mapperJsonData)){
    			...
    			List<ImportResInfoDto> errorList =resInfoService.getErrorResInfos(map);
    			if(errorList != null && errorList.size()>0){
    				...
    				if(user.getIsState() == 0){
    					// 获取个人资源字段缓存
    					lists = cachedService.getPersonFiledSet(user.getOrgId());
    				...
    				// 组装excel文件
    				HSSFWorkbook book = ExcelReader.writeAllInfoToExcel(errorList, mapperList, map1);
    				byte[] content = ExcelUtils.getBytes(book);
    				String fileName = ImportResultEnum.getEnum1(errorCode, ImportResultEnum.getImportResults()).getDescr();
    ...
    public String expTempExcel(HttpServletRequest request,HttpServletResponse response,String mark){
    	try{
			...
			// 组装excel文件
			HSSFWorkbook book = ExcelReader.writeTempToExcel(custFieldSetList,mark);
			byte[] content = ExcelUtils.getBytes(book);
			String fileName =user.getOrgId()+"_import";
			return this.downloadFile(content, fileName+".xls",response);
    	}catch(Exception e){
