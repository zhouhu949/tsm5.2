package com.qftx.tsm.plan.user.day.scontrol;

import com.qftx.base.shiro.ShiroUser;
import com.qftx.base.shiro.ShiroUtil;
import com.qftx.base.util.DateUtil;
import com.qftx.common.cached.CachedService;
import com.qftx.common.domain.BaseJsonResult;
import com.qftx.common.util.StringUtils;
import com.qftx.tsm.cust.service.ComResourceGroupService;
import com.qftx.tsm.plan.CustomFieldUtils;
import com.qftx.tsm.plan.user.day.bean.PlanUserDayResourceBean;
import com.qftx.tsm.plan.user.day.service.PlanUserDayResourceService;
import com.qftx.tsm.plan.user.day.service.PlanUserDayService;
import com.qftx.tsm.plan.year.scontrol.PlanAction;
import org.apache.log4j.Logger;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Controller;
import org.springframework.ui.ModelMap;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.ResponseBody;

import javax.servlet.http.HttpServletRequest;
import java.util.Date;
import java.util.List;

@Controller
@RequestMapping("/plan/day/resource")
public class PlanUserDayResourceAction extends PlanUserDayBaseAction{
	@Autowired
	private PlanUserDayResourceService planUserDayResourceService;
	@Autowired 
	private PlanUserDayService planService;
	@Autowired
	private ComResourceGroupService comResourceGroupService;
	@Autowired
	private CustomFieldUtils customFieldUtils;
	@Autowired
	private CachedService cachedService;
	
	Logger logger=Logger.getLogger(PlanAction.class);
	/*页面加载数据*/
	@RequestMapping("/view")
	public String getResources(HttpServletRequest httpServletRequest,ModelMap model){
		processRequest(httpServletRequest, model);
		return "/plan/dayplan/right/dayPlanDetails_res_right";
	}
	
	@RequestMapping("/resDataJson")
	@ResponseBody
	public BaseJsonResult resDataJson(PlanUserDayResourceBean query,String dateStr){
		try {
			ShiroUser user = ShiroUtil.getShiroUser();
			query.setOrgId(user.getOrgId());
			
			Date planDate = null;
			if(!StringUtils.isBlank(dateStr)){
				planDate = DateUtil.parseDate(dateStr);
			}
			
			if(planDate==null){
				planDate = new Date();
			}
			
			Date nowDate = new Date();
			
			if(StringUtils.isBlank(query.getStatus())){
				query.setStatus(null);
			}
			
			if(StringUtils.isBlank(query.getContactResult())){
				query.setContactResult(null);
			}else{
				query.setContactResult(query.getContactResult().trim());
			}
			
			if(StringUtils.isBlank(query.getQueryStr())){
				query.setQueryStr(null);
			}else{
				query.setQueryStr(query.getQueryStr().trim());
			}
			
			if(StringUtils.isBlank(query.getUserAcc())){
				query.setUserAcc(user.getAccount());
			}
			
			List<PlanUserDayResourceBean> list = null;
			
			if(DateUtil.dateBegin(nowDate).before(DateUtil.dateBegin(planDate))){
				//未来的计划
				query.setPlanDateFrom(DateUtil.dateBegin(planDate));
				query.setPlanDateTo(DateUtil.dateEnd(planDate));
				if(query.getStatus()==null || "0".equals(query.getStatus())||"1".equals(query.getCustLevel())){
					list = planUserDayResourceService.findFromResListPage(query);
				}
			}else{
				list = planUserDayResourceService.queryPageBySudId(query);
			}
			BaseJsonResult succ = BaseJsonResult.success("item", query);
			succ.put("list", list);
			
			return succ;
		} catch (Exception e) {
			logger.error(dateStr,e);
			return  BaseJsonResult.error();
		}
	}
	
	//设置优先||取消优先
	@RequestMapping("/changeCustLevel")
	@ResponseBody
	public String changeCustLevel(String custIdsStr,String level){
		try {
			if(StringUtils.isNotBlank(custIdsStr)&&StringUtils.isNotBlank(level)){
				ShiroUser user = ShiroUtil.getShiroUser();
				planUserDayResourceService.updateCustLevel(user.getOrgId(),user.getAccount() , custIdsStr.split(","), level,user);
				return "success";
			}else{
				return "empty";
			}
		} catch (Exception e) {
			logger.error(e.getMessage(),e);
			return "erro";
		}
	}
	 
	//放弃资源
	@RequestMapping("/giveUp")
	@ResponseBody
	public BaseJsonResult giveUp(String custIdsStr){
		try {
			if(StringUtils.isNotBlank(custIdsStr)){
				ShiroUser user = ShiroUtil.getShiroUser();
				planUserDayResourceService.giveUpResource(user,custIdsStr.split(","));
				return BaseJsonResult.success();
			}else{
				return BaseJsonResult.error("请先选择资源！");
			}
		} catch (Exception e) {
			logger.error(e.getMessage(),e);
			return BaseJsonResult.error();
		}
		
	}
}
