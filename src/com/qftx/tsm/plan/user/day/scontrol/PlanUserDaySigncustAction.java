...
@Controller
@RequestMapping("/plan/day/signcust")
public class PlanUserDaySigncustAction  extends PlanUserDayBaseAction{
	@Autowired
	...
	Logger logger=Logger.getLogger(PlanUserDaySigncustAction.class);
	/*页面加载数据*/
	@RequestMapping("/view")
	public String getResources(HttpServletRequest httpServletRequest,ModelMap model){
		processRequest(httpServletRequest, model);
		return "/plan/dayplan/right/dayPlanDetails_sign_right";
	}
	
	@RequestMapping("/signDataJson")
	@ResponseBody
	public BaseJsonResult signDataJson(PlanUserdaySigncustBean query,DataLableDTO dto,String dateStr){
		try {
			...
			List<PlanUserdaySigncustBean> list = null;
			if(DateUtil.dateBegin(nowDate).before(DateUtil.dateBegin(planDate))){
				...
				if(query.getStatus()==null || "0".equals(query.getStatus())){
					list = planUserdaySigncustService.findFromResListPage(query);
				}
			}else{
				list = planUserdaySigncustService.queryPageBySudId(query);
			}
