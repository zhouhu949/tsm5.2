...
public class HighSearchService{
	Logger logger=Logger.getLogger(HighSearchService.class);
	@Autowired
	...
	static{
		...
		permissionMap.put("followAcc", "isShow");
	...
	static{
		...
		searchMulitMap.put("inputStatus",	"getInputStatus");// 筛选结果
	...
	// 筛选结果
	public List<HighSearchChildDto> getInputStatus(){
		List<HighSearchChildDto> list = new ArrayList<HighSearchChildDto>();
		HighSearchChildDto dto = new HighSearchChildDto();
		dto = new HighSearchChildDto();
		dto.setName("空号");
	...
	public List<HighSearchChildDto>getOptionList(String orgId,String itemCode){
		...
		try{
			//从缓存读取
			List<OptionBean> custTypeList = cachedService.getOptionList(itemCode,orgId);
