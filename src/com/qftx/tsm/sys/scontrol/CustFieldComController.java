 ...
 */
@Controller
@RequestMapping("/custField/com")
public class CustFieldComController{
	Logger logger=Logger.getLogger(CustFieldComController.class);
	 ...
	 * 自定义设置查询 
	 */
	@RequestMapping("/custFieldComPage")
...
//				if(!AppConstant.com_isMajor.equals(cfs.getFieldCode())){ // 重点关注不需显示
...
//			}
//			map.put("rows", rows);
//			request.setAttribute("jsonString", JsonUtil.getJsonString(map));	
		}catch (Exception e) {
			throw new SysRunException(e);
		}
		return "/tsm/sys/custField/custFieldCom";
	}
    ...
    public String modfiy(HttpServletRequest request) { 
		try{
			...
			// 删除缓存
			CachedUtil.getInstance().delete(user.getOrgId() + CachedNames.SEPARATOR + CachedNames.COM_FILEDSET);
			CachedUtil.getInstance().delete(user.getOrgId() + CachedNames.SEPARATOR + CachedNames.COM_FILEDSETS);
	...
	public String reditPage(HttpServletRequest request){
		try{
			...
			return "/tsm/sys/custField/idialog/com_custFiled_opera";
		}catch(Exception e){
    ...
    public String saveUser(HttpServletRequest request) { 
		try{
			...
			custFieldSets.add(custFieldSet);
			custFieldSetService.updateSort(custFieldSets,user.getOrgId()); //40788ee65ca24c7295a0404670d27692
			//修改缓存
			CachedUtil.getInstance().delete(user.getOrgId() + CachedNames.SEPARATOR + CachedNames.COM_FILEDSET);
			CachedUtil.getInstance().delete(user.getOrgId() + CachedNames.SEPARATOR + CachedNames.COM_FILEDSETS);
			 return AppConstant.RESULT_SUCCESS;
