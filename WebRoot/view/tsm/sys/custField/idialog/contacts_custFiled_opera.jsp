...
<title>系统设置-系统字段设置（企业客户--联系人）-添加自定义字段</title>
...
$(function(){
  ...
  $(".sure-btn").click(function(){
	  var $val = $("#fieldName").val();
		...
		var url = "${ctx}/custField/contacts/fieldNameIsExists.do";
		$.ajax({
			url : url,
			data: {'fieldName':$val},
			type : 'post',
...
function ajaxSubmit_(){
	...
	$('#operaForm').ajaxSubmit({
		url : '${ctx}/custField/contacts/opera.do',
		type : 'post',
		...
		success : function(data) {	
			if(data == 0){			
				...
				if(data == 4){
					window.top.iDialogMsg("提示","日期类型的自定义字段最多为1个");
				}else if(data == 3){
					window.top.iDialogMsg("提示","无法将字段类型变更为日期类型");
				}else if(data == 5){
					window.top.iDialogMsg("提示","非日期类型的自定义字段最多为4个");
				}else{
...
<%-- 		<input type="hidden" id="isQuery" name="isQuery" value="${item.isQuery }">
		<input type="checkbox" class="fl_l" id="isQuery_check" disabled="disabled" <c:if test="${item.isQuery eq 1 }">checked</c:if> />
	...
	<p class='bomp-p'>
		...
		<select class="sel_a" name="enable" <c:if test="${item.fieldCode eq 'name' or item.fieldCode eq 'telphone' or item.fieldCode eq 'telphonebak'}">disabled="disabled"</c:if>>
