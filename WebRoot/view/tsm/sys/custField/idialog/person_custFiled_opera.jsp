...
<title>系统设置-系统字段设置（个人客户）-添加自定义字段</title>
...
$(function(){
    ...
    $(".sure-btn").click(function(){
    	...
    	var url = "${ctx}/custField/person/fieldNameIsExists.do";
    	$.ajax({
    		url : url,
    		data: {'fieldName':$val},
    		type : 'post',
...
function ajaxSubmit_(){
	...
	$('#operaForm').ajaxSubmit({
		url : '${ctx}/custField/person/opera',
		type : 'post',
...
<%-- 		<input type="hidden" id="isQuery" name="isQuery" value="${item.isQuery }">
		<input type="checkbox" class="fl_l" id="isQuery_check" <c:if test="${isDefined eq 1 or count >= 5 }">disabled="disabled"</c:if><c:if test="${item.isQuery eq 1 }">checked</c:if> />
		<label class='lab_a fl_l'>查询条件</label> --%>
	</p>
	<p class='bomp-p'  id="bomp_">
		<label class='lab_a fl_l'>字段名称：</label><input type='text' id="fieldName" maxlength="6"  name="fieldName" value="${item.fieldName }" <c:if test="${isDefined eq 1 }">disabled="disabled"</c:if> class='ipt_a fl_l' />
	...
	<p class='bomp-p'>
		...
		<select class="sel_a" name="enable" <c:if test="${item.fieldCode eq 'name' or item.fieldCode eq 'company'
					 or item.fieldCode eq 'unithome' or item.fieldCode eq 'mobilephone' or item.fieldCode eq 'telphone' }">disabled="disabled"</c:if>>
