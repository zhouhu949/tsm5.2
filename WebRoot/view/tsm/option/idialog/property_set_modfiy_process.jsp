...
<title>系统设置-系统属性设置-销售进程设计-修改销售进程</title>
...
function submit_(){
	$('#setForm').ajaxSubmit({
			url : '${ctx}/opt/set/addOrEditOption.do',
			type : 'post',
...
function checkIsNull(){
	...
	$('[checkProp^="chk_"]').each(function() {
		...
		if($this.val() != null && $.trim($this.val()) != ""){
			...
			$("#error_"+id).text("");
			/* if(id == "optionName"){//对名称的特殊处理 
				if( $this.val().length >10 ){//如果名称长度超过10 则提示
					isTrue = false;
					$("#bomp_error_"+id).addClass("bomp-error");
					$("#error_"+id).text("最多输入10个字！");
					opera = true;
				}else{//如果名称长度未超过10  就取消错误 
					$("#bomp_error_"+id).removeClass("bomp-error");
					$("#error_"+id).text("");
				}
			} */
...
<div class='bomp-cen'>
	<form id="setForm" method="post">
		...
		<div class='bomp-p' id="bomp_error_optionName">
			...
			<input type='text'  name="optionName" id="optionName" value='${item.optionName }'  maxlength="10"  class='ipt_a fl_l' checkProp="chk_" placeholder="最多10个汉字"/>
		...
		<p class="fl_l" style="margin:20px 0 0 40px;">
			<label for=""><span class="font-color" >温馨提示：</span><span>客户成熟度的高低由排序值大小表示，排序值越大成熟度越高。</span></label>
		</p>
		<div class='bomb-btn'>
