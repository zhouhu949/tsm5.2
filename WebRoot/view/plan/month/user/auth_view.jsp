...
<script type="text/javascript" src="${ctx}/static/js/view/plan/month/user/auth_view.js"></script>
...
function showDetail(planId,userName){
	window.parent.pubDivShowIframe('detailWindow','${ctx}/plan/month/user/detailWindow?planId='+planId,'计划详情（'+userName+'）',900,510);
}
...
<body>
<form action="${ctx }/plan/month/user/authView${_v}">
		...
		<div class="com-search-box fl_l">
		  <!--<p class="search clearfix"><input class="input-query fl_l" type="text" value="" /><label class="hide-span"></label></p> -->
		   <div class="list-box">
				...
				<div class="reso-sub-dep fl_l">
					<input class="owner-sour" type="text" value="提交人" style="margin-top:5px;">
					<div class="manage-owner-sour">
						<div class="sub-dep-ul">
							<ul id="userTree" class="easyui-tree" > </ul>
			    		</div>
					    <div class="sure-cancle clearfix" style="width:120px">
							<a id="confirmUserTree" class="com-btnc bomp-btna com-btna-sty reso-owner-sure fl_l" href="javascript:;"><label>确定</label></a>
							<a id="clearUserTree" class="com-btnd bomp-btna fl_l" id="close02" href="javascript:;"><label>清空</label></a>
						</div>
					</div>
				</div>
			</div>
	...
	<div class="com-table com-mta hyx-cm-table fl_l" style="display:block;">
		<table width="100%" cellspacing="0" cellpadding="0" class="sty-borcolor-a sty-borcolor-c com-table-b com-radius">
			...
			<tbody>
				...
				<c:when test="${!empty list}">
					<c:forEach var="bean" items="${list}" varStatus="i">
						...
						<c:if test="${i.index%2==0}"><tr></c:if>
							...
							<a href="javascript:showDetail('${bean.id}','${bean.userName}');" class="icon-detail" title="查看"></a>
							</div></td>
				...
				<c:otherwise>
					<tr class="no_data_tr"><td colspan="20" style="text-align: center;"><b>当前列表无数据！</b></td></tr>
				</c:otherwise>
			    </c:choose>
			</tbody>
		</table>
