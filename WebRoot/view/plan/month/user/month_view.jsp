...
<title>销售计划管理-月度计划（个人）-月度计划查看（个人未执行）</title>
		...
		<table width="100%" cellspacing="0" cellpadding="0" class="sty-borcolor-a sty-borcolor-c com-table-a">
			...
			<tbody>
					...
					<c:when test="${dto!=null and !empty dto.custs}">
						<c:forEach items="${dto.custs}" var="cust" varStatus="i">
							...
							<c:if test="${i.index%2==0}"><tr></c:if>
								...
								<td><div class="overflow_hidden w80" title="${cust.planMoney }">${cust.planMoney }</div></td>
				...
				<tr class="sty-bgcolor-b">
					...
					<td><div class="overflow_hidden w100" title="${dto.custCount.planMoney}">${dto.custCount.planMoney}</div></td>
