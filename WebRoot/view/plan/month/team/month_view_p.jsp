...
<title>销售计划管理-团队月度计划-查看页面（已执行）</title>
...
function showDetail(planId,userName){
	pubDivShowIframe('detailWindow','${ctx}/plan/month/user/detailWindow_p?planId='+planId,'计划详情（'+userName+'）',900,510);
}
...
<body> 
<form id="monthPlanForm" action="${ctx}/plan/month/team/monthView_p">
	<input id="id" name="id" type="hidden" value="${plan.id}"/>
</form>
<div class="com-conta">
	<h2 class="hyx-mpe-h2">${plan.planYear}年${plan.groupName}${plan.planMonth}月份计划执行情况</h2>
	<p class="hyx-mpe-p">
		<label class="lab fl_l sty-bgcolor-a sty-color-a sty-borcolor-d com-radius">执行/计划汇总</label>
	</p>
					...
					<c:when test="${userPlans!=null and !empty userPlans}">
						<c:forEach items="${userPlans}" var="plan" varStatus="i">
							...
							<c:if test="${i.index%2==0}"><tr></c:if>
								...
								<td><div class="overflow_hidden w70" title="${plan.factWillcustnum}/${plan.planWillcustnum}">${plan.factWillcustnum}/${plan.planWillcustnum}</div></td>
								<td><div class="overflow_hidden w70" title="${plan.factSigncustnum}/${plan.planSigncustnum}">${plan.factSigncustnum}/${plan.planSigncustnum}</div></td>
								<td><div class="overflow_hidden w70" title="${plan.factMoney}/${plan.planMoney}">${plan.factMoney}/${plan.planMoney}</div></td>
								<c:choose>
									<c:when test="${plan.planStatus == 0}"><td><div class="overflow_hidden w120" title="未完成">未完成</div></td></c:when>
									<c:when test="${plan.planStatus == 1 or plan.planStatus == 2}"><td><div class="overflow_hidden w120" title="完成">完成</div></td></c:when>
								</c:choose>
