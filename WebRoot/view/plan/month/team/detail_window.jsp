					...
					<c:when test="${userPlans!=null and !empty userPlans}">
						<c:forEach items="${userPlans}" var="userPlan" varStatus="i">
							...
							<c:if test="${i.index%2==0}"><tr></c:if>  
								...
								<td><div class="overflow_hidden w80" title="${userPlan.planWillcustnum}">${userPlan.planWillcustnum}</div></td>
								<td><div class="overflow_hidden w80" title="${userPlan.planSigncustnum}">${userPlan.planSigncustnum}</div></td>
								<td><div class="overflow_hidden w80" title="${userPlan.planMoney}">${userPlan.planMoney}</div></td>
								<c:choose>
			...
			<tbody>              
				<tr>
					<c:choose>
						...
						<c:when test="${plan.authState == 1}"><td><div class="overflow_hidden w8" title="待审核">待审核</div></td></c:when>
