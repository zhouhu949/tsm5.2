    ...
    $(".sure-btn").click(function(){
    	...
    	if(dContext!=null&&dContext.replace(/(^\s+)|(\s+$)/g,'').length>0){
    		...
    		if(dContext.length<=500){
    			$.post(ctx+"/plan/month/team/commont",{"planId":planId,"context":dContext},
    					function(data){
    				if("success"==data.status){
    					setTimeout('window.parent.$("#iframepage")[0].contentWindow.refreshPage();',20);
    					setTimeout('closeParentPubDivDialogIframe("commontWindow");',50);
    					//closeParentPubDivDialogIframe("commontWindow");
    				}else{
    				}
    			}); 
    		}else{
    			window.parent.wrongDivDialog("erro","评论内容最大长度为500字!");
    		}
    	}else{
    		window.parent.wrongDivDialog("erro","评论内容不能为空!");
    	}
    });
