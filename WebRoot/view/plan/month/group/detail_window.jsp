...
<title>销售计划管理-月度计划（部门编辑）-计划详情（未执行）</title>
					...
					<c:when test="${plans!=null and !empty plans}">
						<c:forEach items="${plans}" var="teamPlan" varStatus="i">
							...
							<c:if test="${i.index%2==0}"><tr></c:if>  
								...
								<td><div class="overflow_hidden w110" title="${teamPlan.planWillcustnum}">${teamPlan.planWillcustnum}</div></td>
								<td><div class="overflow_hidden w110" title="${teamPlan.planSigncustnum}">${teamPlan.planSigncustnum}</div></td>
								<td><div class="overflow_hidden w110" title="${teamPlan.planMoney}">${teamPlan.planMoney}</div></td>
								<c:choose>
