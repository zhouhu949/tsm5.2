...
<title>销售计划管理-月度计划（部门编辑）-编辑页面（部门）-偏差值编辑</title>
<!--公共样式-->
...
$(function(){
    ...
    $("input").blur(function(){
		...
		if(check(_this)){
	    	...
	    	if(val=="+"){
	    		$($(_this).siblings(".mte_a")[1]).text( (parseFloat($($(_this).siblings(".mte_a")[0]).text())*100 +parseFloat($(_this).val())*100 )/100);
	    	}else{
	    		$($(_this).siblings(".mte_a")[1]).text( (parseFloat($($(_this).siblings(".mte_a")[0]).text())*100 -parseFloat($(_this).val())*100 )/100);
	    	}
		}
    ...
    $(".sel").change(function(){
    	...
    	if(val=="+"){
    		...
    		$($(_this).siblings(".mte_a")[1]).text( (parseFloat($($(_this).siblings(".mte_a")[0]).text())*100 +parseFloat($($(_this).siblings("input")).val())*100 )/100);
    	}else{
...
<div class='bomp-cen'>
	<p class="bomp-p bomp-mteef-p warp-plan">
		...
		<span class="bomp-red bomp-mteef-tip">必填项！</span>
	</p>
	<p class="bomp-p bomp-mteef-p warp-plan">
	...
	<p class="bomp-p bomp-mteef-p warp-plan">
		...
		<label class="mte_b">=</label>
		<label class="mte_a">${plan.usersPlanMoney+plan.warpMoney}</label>
