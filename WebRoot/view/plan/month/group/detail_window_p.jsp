...
<title>销售计划管理-月度计划（部门编辑）-计划详情（已执行）</title>
					...
					<c:when test="${plans!=null and !empty plans}">
						<c:forEach items="${plans}" var="teamPlan" varStatus="i">
							...
							<c:if test="${i.index%2==0}"><tr></c:if>  
								<td><div class="overflow_hidden w110" title="${teamPlan.groupName}">${teamPlan.groupName}</div></td>
								<td><div class="overflow_hidden w110" title="${teamPlan.factWillcustnum}/${teamPlan.planWillcustnum}">${teamPlan.factWillcustnum}/${teamPlan.planWillcustnum}</div></td>
								<td><div class="overflow_hidden w110" title="${teamPlan.factSigncustnum}/${teamPlan.planSigncustnum}">${teamPlan.factSigncustnum}/${teamPlan.planSigncustnum}</div></td>
								<td><div class="overflow_hidden w110" title="${teamPlan.factMoney}/${teamPlan.planMoney}">${teamPlan.factMoney}/${teamPlan.planMoney}</div></td>
								<c:choose>
									<c:when test="${teamPlan.planStatus == 0}"><td><div class="overflow_hidden w110" title="未完成">未完成</div></td></c:when>
									<c:when test="${teamPlan.planStatus == 1 or teamPlan.planStatus == 2}"><td><div class="overflow_hidden w110" title="完成">完成</div></td></c:when>
								</c:choose>
								<c:choose>
