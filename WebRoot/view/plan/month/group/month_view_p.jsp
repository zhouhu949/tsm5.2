...
function showDetail(planId,groupName,type){
	window.parent.showDetail(planId,groupName,type,false);
}
...
<body> 
<form id="monthPlanForm" action="${ctx}/plan/month/group/monthView_p${_v}">
		...
		<table width="100%" cellspacing="0" cellpadding="0" class="sty-borcolor-a sty-borcolor-c com-table-a">
			...
			<tbody>              
					...
					<c:when test="${plans!=null and !empty plans}">
						<c:forEach items="${plans}" var="teamPlan" varStatus="i">
							...
							<c:if test="${i.index%2==0}"><tr></c:if>
								...
								<td><div class="overflow_hidden w70" title="${teamPlan.factWillcustnum}/${teamPlan.planWillcustnum}">${teamPlan.factWillcustnum}/${teamPlan.planWillcustnum}</div></td>
								<td><div class="overflow_hidden w70" title="${teamPlan.factSigncustnum}/${teamPlan.planSigncustnum}">${teamPlan.factSigncustnum}/${teamPlan.planSigncustnum}</div></td>
								<td><div class="overflow_hidden w70" title="${teamPlan.factMoney}/${teamPlan.planMoney}">${teamPlan.factMoney}/${teamPlan.planMoney}</div></td>
								<c:choose>
									<c:when test="${teamPlan.planStatus == 0}"><td><div class="overflow_hidden w120" title="未完成">未完成</div></td></c:when>
									<c:when test="${teamPlan.planStatus == 1 or plan.planStatus == 2}"><td><div class="overflow_hidden w120" title="完成">完成</div></td></c:when>
								</c:choose>
				...
				<tr class="sty-bgcolor-b">
					...
					<td><div class="overflow_hidden w110" title="${teamPlansSum.factWillcustnum}/${teamPlansSum.planWillcustnum}">${teamPlansSum.factWillcustnum}/${teamPlansSum.planWillcustnum}</div></td>
					<td><div class="overflow_hidden w110" title="${teamPlansSum.factSigncustnum}/${teamPlansSum.planSigncustnum}">${teamPlansSum.factSigncustnum}/${teamPlansSum.planSigncustnum}</div></td>
					<td><div class="overflow_hidden w110" title="${teamPlansSum.factMoney}/${teamPlansSum.planMoney}">${teamPlansSum.factMoney}/${teamPlansSum.planMoney}</div></td>
		...
		<table width="100%" cellspacing="0" cellpadding="0" class="sty-borcolor-a sty-borcolor-c com-table-a">
			...
			<tbody>              
				<tr>
					...
					<td><c:choose>
						<c:when test="${plan.planStatus == 0}"><div class="overflow_hidden w110" title="未完成">未完成</div></c:when>
						<c:when test="${plan.planStatus == 1 or plan.planStatus == 2}"><div class="overflow_hidden w110" title="完成">完成</div></c:when>
					</c:choose></td>
