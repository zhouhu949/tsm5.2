...
<script type="text/javascript" src="${ctx}/static/js/view/plan/dailyplan/right/dailyPlan_detail_will.js${_v}"></script>
<title>日计划详情</title>
...
<body>
	<div class="hyx-layout">
		<div class="hyx-dayplan-center hyx-layout-content">
			<form id="myForm" method="post">
				...
				<input type="hidden" id="isMajor" name="isMajor" value=""><!-- 重点客户 -->
		   ...
		   <div class="com-search hyx-cfu-search new-detail-com-search">
			...
			<p class="com-moTag"><label class="a">数据分类：</label>
				  	...
				  	<a class="e"  href="javascript:;" data-result ="2">意向变更</a>
				  	<a class="e"  href="javascript:;" data-result ="3">转公海</a>
				  	<a class="e"  href="javascript:;" data-result ="4">意向无变化</a>
				  	<a class="e ${status eq '1' ?'e-hover':''}"  href="javascript:;" data-result ="5">已联系</a>
				  	<a class="e ${status eq '0' ?'e-hover':''}"  href="javascript:;" data-result ="6">未联系</a>
				  	<!-- <a class="e"  href="javascript:;" data-result ="7">重点客户</a> -->
			</p>
			<div class="com-btnlist hyx-cfu-btnlist fl_l">
				<c:if test="${ isMine && todayFlag ne -1 }">
					<a href="javascript:;" class="com-btna fl_l" id="join_cust" data-saleprocessid="${custStatusId }" data-planid="${sudId }" data-dateStr="${dateStr }"><label>加入客户</label></a>
				...
				<table id="t1" width="100%" cellspacing="0" cellpadding="0" class="sty-borcolor-a sty-borcolor-c com-table-b com-radius ajax-table">
					<thead>
						<tr class="sty-bgcolor-b" role="head">
							...
							<th><span class="sp sty-borcolor-b">创建时间</span></th>
  ...
  <tr class="hyx-fur-tr {{even_odd @index}} {{dayPlanDis permision 'disabled'}}" data-followid="{{cust_last_cust_follow_id}}" data-custId="{{custId}}">
    ...
    <td>
	<div class="w150 overflow_hidden">
    	<c:choose>
		...
		<c:if test="${todayFlag ne -1}">
		{{#compare isMajor "==" 0}}
			<a href='javascript:;' id='setMajor_{{custId}}' class='icon-nofocus {{typeDayPlanDis permision "img-gray" typeChange}}' title='重点关注'></a>
		{{/compare}}	
		{{#compare isMajor "==" null}}
			<a href='javascript:;' id='setMajor_{{custId}}' class='icon-nofocus {{typeDayPlanDis permision "img-gray" typeChange}}' title='重点关注'></a>
		{{/compare}}	
		{{#compare isMajor "==" 1}}
			<a href='javascript:;' id='cancleMajor_{{custId}}' class='icon-focus-atten {{typeDayPlanDis permision "img-gray" typeChange}}' title='取消关注'></a>
		{{/compare}}
		</c:if>
		<c:if test="${isMine && todayFlag ne -1}">
			<a href='javascript:;' id='sign_{{custId}}' signSetting='{{signSetting}}' class='icon-sign-cont {{typeDayPlanDis permision "img-gray" typeChange}}' title='签约'></a>
			<a href='javascript:;' id='dele_{{custId}}_2'  class='icon-dele {{typeDayPlanDis permision "img-gray" typeChange}}' title='放弃客户'></a>
		</c:if>
		</div>
    </td>
    <td>
	...
	<td>
    <div class="w70 overflow_hidden" title="{{${isState eq 1 ? 'custUser' : 'custName'}}}"> {{${isState eq 1 ? 'custUser' : 'custName'}}}</div>
    </td>
 <td>
    <div class="w80 overflow_hidden" title="{{sourceStr}}"> {{sourceStr}}</div>
    </td>
    <td>
     <div class="w150 overflow_hidden" title="{{custInputtimeStr}}"> {{custInputtimeStr}}</div>
    </td>
	<td>
     <div class="w150 overflow_hidden" title="{{taoTimeStr}}"> {{taoTimeStr}}</div>
    </td>
	<td>
      <div class="w150 overflow_hidden" title="{{actionDateStr}}">{{actionDateStr}}</div>
    </td>
	<td>
      <div class="w130 overflow_hidden " title="{{custStatus}}">{{custStatus}}</div>
    </td>
    <td>
