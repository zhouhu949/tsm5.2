...
<head>
	...
	<script type="text/javascript" src="${ctx}/static/js/view/plan/day/signcust_add.js${_v}"></script>
...
/*时间弹框*/
	...
	$('#d2').dateRangePicker({
    	...
    	if(obj.date1 != 'Invalid Date'){
    		$('#startContractEndDate').val(moment(obj.date1).format('YYYY-MM-DD'));
    	}
    	if(obj.date2 != 'Invalid Date'){
    		$('#endContractEndDate').val(moment(obj.date2).format('YYYY-MM-DD'));
    	}
    	var s = $(this).parents('.select');
    	var dt=s.children("dt");
        var dd=s.children("dd");
...
function setPdate(id,type){
	...
	if(type==0){
		if('oDateType'==id){
			...
			$("#startContractEndDate").val('');
			$("#endContractEndDate").val('');
		}
	}
}
function doQuery(){
...
<body>
<form id="addResourceForm" action="${ctx}/plan/day/signcust/signcustAddWindow" method="post">
		...
		<div class="com-search-box fl_l">
		   ...
		   <div class="list-box">
			   ...
			   <!-- 下次联系时间 -->
				...
				<!--合同失效日期 -->
				<input type="hidden" name="startContractEndDate" id="startContractEndDate" value="${item.startContractEndDate }" />
				<input type="hidden" name="endContractEndDate" id="endContractEndDate" value="${item.endContractEndDate }" />
				<input type="hidden" name="lDateType" id="lDateType" value="${item.lDateType }" />
				<c:set var="lDateName" value="合同失效日期" />
				<c:choose>
					...
					<c:when test="${item.lDateType eq 5 }">
						<c:set var="lDateName" value="${item.startContractEndDate.replaceAll('^20','').replace('-','.') }/${item.endContractEndDate.replaceAll('^20','').replace('-','.') }" />
					</c:when>
				</c:choose>
				...
				<c:when test="${!empty list}">
					<c:forEach var="bean" items="${list}" varStatus="i">
						...
						<c:if test="${i.index%2==0}"><tr></c:if>
							...
							<td><div class="overflow_hidden w110" title="${bean.custEndtimeStr}">${bean.custEndtimeStr}</div></div></td>
