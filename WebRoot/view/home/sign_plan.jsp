...
<!-- 			<th><span class="sp sty-borcolor-b">下次联系时间</span></th> -->
...
<%-- 										<c:if test="${!empty bean.ownerStartDate }"> --%>
											<a href="###" btn-type="auth" auth_id="base_followCust" <c:if test="${shiroUser.serverDay gt 0}">id="follow_${bean.custId }" custType="3"</c:if> class="icon-follow-up ${shiroUser.serverDay gt 0 ? '' : 'img-gray' }" title="跟进"></a>
											<a href="###" btn-type="auth" auth_id="mySignCust_addContract" <c:if test="${shiroUser.serverDay gt 0}">id="addContract_${bean.custId }"</c:if> class="icon-new-contr ${shiroUser.serverDay gt 0 ? '' : 'img-gray' }" title="新增合同"></a>
<%-- 										</c:if> --%>
...
<%-- 								<td><div class="overflow_hidden w130" title="<fmt:formatDate value="${bean.ownerStartDate }" pattern="yyyy-MM-dd HH:mm:ss"/>"><fmt:formatDate value="${bean.ownerStartDate }" pattern="yyyy-MM-dd"/></div></td> --%>
								<td>
									<div class="overflow_hidden w90 link">
										<a href="###" btn-type="auth" auth_id="base_followCust" <c:if test="${shiroUser.serverDay gt 0}">id="follow_${bean.resCustId }" custType="3"</c:if> class="icon-follow-up ${shiroUser.serverDay gt 0 ? '' : 'img-gray' }" title="跟进"></a>
										<a href="###" btn-type="auth" auth_id="mySignCust_addContract" <c:if test="${shiroUser.serverDay gt 0}">id="addContract_${bean.resCustId }"</c:if> class="icon-new-contr ${shiroUser.serverDay gt 0 ? '' : 'img-gray' }" title="新增合同"></a>
