			...
			$('#saveBtn').click(function(){
				...
				$.ajax({
					...
					success:function(data){
						...
						if(data == 1){
							window.top.iDialogMsg("提示","保存联系人成功！");
							var getTimestamp=new Date().getTime();  
							...
							window.top.iDialogMsg("提示","保存联系人失败！");
							var getTimestamp=new Date().getTime();  
							 $('li[name$=toSearchRes]').addClass('li_click').siblings('li').removeClass('li_click');
							 window.parent.$('#iframepage').attr('src',ctx+'/res/incall/toSearchRes?custId='+$('#rciId').val()+'&v='+getTimestamp);
						}
...
<body>
<form action="${ctx}/res/cust/saveDetail" method="post" id="myForm">
<div class='bomp-cen bomp_change bomp-mcl'>
	<div class='bomp_tit bomp_tit_a skin-minimal'><label class='lab'>新增联系人</label>
	<input type="hidden" name="tscidId" value="${detail.tscidId }" />
    <input type="hidden" id="isDefault" name="isDefault" value="${!empty detailList?'0':'1' } " />
...
　　　<input type="hidden" name='uhValiDateType' id='uhValiDateType' value='${uhValiDateType }'>
    ...
    <input type="hidden" name='tanpin' id='tanpin' value='1'>		
	      <div class="check fl_r"><input type='checkbox' name="mainLink" class='fl_l' ${!empty detailList?'':'checked disabled' } id="mainLink"/><label class='fl_l font-s'>主要联系人</label></div></div>
			             ...
			             <c:when test="${'name' eq field.fieldCode && field.enable eq 1  }">
								<p class='bomp-p' id="bomp_error_${field.fieldCode }">
											   	<label class='lab_a fl_l'>
											    	...
											    	${field.fieldName}：
											   	</label><input type='text'   id="${field.fieldCode }"  name="${field.fieldCode }"  value=''  checkDetailProp="${field.isRequired eq 0?'':'chk_1_1' }" class='ipt_a fl_l' />
			             ...
			             <c:when test="${'telphone' eq field.fieldCode && field.enable eq 1  }">
								<p class='bomp-p' id="bomp_error_${field.fieldCode }">
									<label class='lab_a fl_l'>
									 ...
									 <input type='hidden'   id="${field.fieldCode }"  name="${field.fieldCode }"  value='${detail.telphone }' checkDetailProp="${field.isRequired eq 0?'chk_1_0':'chk_1_1' }"  class='ipt_a fl_l' />
									 ...
									 <input type='text'   id="${field.fieldCode }_safe"  name="${field.fieldCode }_safe"  value='${detail.telphone }' checkDetailProp="${field.isRequired eq 0?'chk_1_0':'chk_1_1' }"  class='ipt_a fl_l' />
									 <span class="error" id="error_${ field.fieldCode}"></span>
								</p>		               
			             </c:when>
			             <c:when test="${'telphonebak' eq field.fieldCode && field.enable  eq 1  }">
								<p class='bomp-p' id="bomp_error_${field.fieldCode }">
									<label class='lab_a fl_l'>
									 ...
									 ${field.fieldName}：</label>
									 <input type='hidden'   id="${field.fieldCode }"  name="${field.fieldCode }"  value='${detail.telphonebak }' checkDetailProp="${field.isRequired eq 0?'chk_1_0':'chk_1_1' }"  class='ipt_a fl_l' />
									 ...
									 <input type='text'   id="${field.fieldCode }_safe"  name="${field.fieldCode }_safe"  value='${detail.telphonebak }' checkDetailProp="${field.isRequired eq 0?'chk_1_0':'chk_1_1' }"  class='ipt_a fl_l' />
									 <span class="error" id="error_${ field.fieldCode}"></span>
								</p>		               
			             </c:when>
			             <c:when test="${'sex' eq field.fieldCode && field.enable eq 1  }">
								<p class='bomp-p' id="bomp_error_${field.fieldCode }">
									<label class='lab_a fl_l'>
									 ...
									 ${field.fieldName}：</label>
									    <select class='sel_a fl_l'  id="${field.fieldCode }"  name="${field.fieldCode }"  value='${detail.sex }'  checkDetailProp="${field.isRequired eq 0?'':'chk_2_1' }" >
			             ...
			             <c:when test="${'birthday' eq field.fieldCode && field.enable  eq 1  }">
								<p class='bomp-p' id="bomp_error_${field.fieldCode }">
									<label class='lab_a fl_l'>
									 ...
									 ${field.fieldName}：</label><input type='text'   onclick="WdatePicker({maxDate:'%y-%M-%d'})"  id="${field.fieldCode }"  name="${field.fieldCode }"  value='<fmt:formatDate value="${detail.birthday}" pattern="yyyy-MM-dd"/>'  checkDetailProp="${field.isRequired eq 0?'':'chk_5_1' }"  class='ipt_a fl_l' />
									 <span class="error" id="error_${ field.fieldCode}"></span>
								</p>		               
			             </c:when>
			             <c:when test="${'email' eq field.fieldCode && field.enable  eq 1  }">
								<p class='bomp-p' id="bomp_error_${field.fieldCode }">
									<label class='lab_a fl_l'>
									 ...
									 ${field.fieldName}：</label><input type='text'   id="${field.fieldCode }"  name="${field.fieldCode }"  value='${detail.email }' checkDetailProp="${field.isRequired eq 0?'chk_1_0':'chk_1_1' }"  class='ipt_a fl_l' />
									 <span class="error" id="error_${ field.fieldCode}"></span>
								</p>		               
			             </c:when>
			             <c:when test="${'fax' eq field.fieldCode && field.enable  eq 1  }">
								<p class='bomp-p' id="bomp_error_${field.fieldCode }">
									<label class='lab_a fl_l'>
									 ...
									 ${field.fieldName}：</label><input type='text'   id="${field.fieldCode }"  name="${field.fieldCode }"  value='${detail.fax }' checkDetailProp="${field.isRequired eq 0?'chk_1_0':'chk_1_1' }"  class='ipt_a fl_l' />
									 <span class="error" id="error_${ field.fieldCode}"></span>
								</p>		               
			             </c:when>
			             <c:when test="${'qq' eq field.fieldCode && field.enable  eq 1  }">
								<p class='bomp-p' id="bomp_error_${field.fieldCode }">
									<label class='lab_a fl_l'>
									 ...
									 ${field.fieldName}：</label><input type='text'   id="${field.fieldCode }"  name="${field.fieldCode }"  value='${detail.qq }' checkDetailProp="${field.isRequired eq 0?'':'chk_1_1' }"  class='ipt_a fl_l' />
									 <span class="error" id="error_${ field.fieldCode}"></span>
								</p>		               
			             </c:when>	
			             ...
			             <c:when test="${'wangwang' eq field.fieldCode && field.enable  eq 1  }">
								<p class='bomp-p' id="bomp_error_${field.fieldCode }">
									<label class='lab_a fl_l'>
									 ...
									 ${field.fieldName}：</label><input type='text'   id="${field.fieldCode }"  name="${field.fieldCode }"  value='${detail.wangwang }' checkDetailProp="${field.isRequired eq 0?'':'chk_1_1' }"  class='ipt_a fl_l' />
									 <span class="error" id="error_${ field.fieldCode}"></span>
								</p>		               
			             </c:when>
			             <c:when test="${'work' eq field.fieldCode && field.enable  eq 1  }">
								<p class='bomp-p' id="bomp_error_${field.fieldCode }">
									<label class='lab_a fl_l'>
									 ...
									 ${field.fieldName}：</label><input type='text'   id="${field.fieldCode }"  name="${field.fieldCode }"  value='${detail.work }' checkDetailProp="${field.isRequired eq 0?'':'chk_1_1' }"  class='ipt_a fl_l' />
									 <span class="error" id="error_${ field.fieldCode}"></span>
								</p>		               
			             </c:when>
			             <c:when test="${'groupname' eq field.fieldCode && field.enable  eq 1  }">
								<p class='bomp-p' id="bomp_error_${field.fieldCode }">
									<label class='lab_a fl_l'>
									 ...
									 ${field.fieldName}：</label><input type='text'   id="${field.fieldCode }"  name="${field.fieldCode }"  value='${detail.groupname }' checkDetailProp="${field.isRequired eq 0?'':'chk_1_1' }"  class='ipt_a fl_l' />
									 <span class="error" id="error_${ field.fieldCode}"></span>
								</p>		               
			             </c:when>
			            </c:choose>
