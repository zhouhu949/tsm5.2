...
$(function(){
	window.onload=function(){
		var height = $(".bomp_change").height()+10;
...
<body>
       ...
       <c:set var= "mark" value="0"></c:set>
          <div class='bomp-cen bomp_change bomp_ica'>
            ...
            <div class='bomp_tit bomp_tit_a'><label class='lab'>企业信息</label>
...
<%-- 					<a href="###" class="${'1' eq custInfoBean.isMajor ? 'icon-focus-atten':'icon-nofocus'}" id="majorId" major="${custInfoBean.isMajor }"></a> --%>
				...
				<c:if test="${CommonOwnerEditAuth eq 1}"><label class="btn"><a href="###" data-authority="base_custEdit" class="icon-edit bomp_ica_editbtn" id="editId"></a></label></c:if>
					
							
			</div>
			<div class="bomp_ica_save">
<!-- 				<p class='bomp-p'> -->
<%-- 					<label class='lab_a fl_l'>客户名称：</label><span class="fl_l" style="display:inline-block;height:30px;line-height:30px;width:260px;overflow:hidden;">${custInfoBean.name }</span> --%>
<!-- 				</p> -->
									  ...
									  <c:set var="mark" value="${mark+1 }" />
										<p class="bomp-p">
											<label class='lab_a fl_l'>${fieldSet.fieldName}：</label>
											   ...
											   <c:choose>
											   <c:when test="${custInfoBean[fieldSet.fieldCode] eq option.optionlistId}">${option.optionName }</c:when>
										...
										<p class="bomp-p">
											...
											<c:choose>
											   <c:when test="${  fieldSet.fieldCode  eq 'mobilephone' or  fieldSet.fieldCode  eq 'telphone'  or  fieldSet.fieldCode  eq 'alternatePhone2'  or  fieldSet.fieldCode  eq 'telphonebak'}">
		                                            <span phone='tel' title="${custInfoBean[fieldSet.fieldCode]}" class="fl_l" style="display:inline-block;height:30px;line-height:28px;width:260px;overflow:hidden;font-size:12px;">${custInfoBean[fieldSet.fieldCode]}</span>												   
											   </c:when>
			...
			<div class='bomp_tit bomp_tit_a'><label class='lab'>联系人信息</label>
				<div class="add">
				<c:if test="${CommonOwnerEditAuth eq 0}"></c:if>
				<c:if test="${CommonOwnerEditAuth eq 1}">
				<label data-authority="base_custEdit" class="add-label"><b>+</b>
				<a data-authority="base_custEdit" href='###' id='addDetail'>新增联系人</a></label>
				</c:if>
			...
			<!-- 新增联系人 结束-->
			<div class='com-table bomp-table-a fl_l' style="width:89%;margin-left:30px;padding-bottom:40px;">
							...
							<c:when test="${!empty details }">
								<c:forEach items="${details }" var="detail">
									<tr>
										...
										<td><div class='overflow_hidden w150' title='${detail.telphone }'  phone='tel'>${detail.telphone }</div></td>
										<td>
											<div class="bomp-edit">
												<c:if test="${CommonOwnerEditAuth eq 0}"></c:if>
				                                <c:if test="${CommonOwnerEditAuth eq 1}"><a data-authority="base_custEdit" href="###" id="edti_${detail.tscidId }" class='lk'>编辑</a></c:if>				                               	
											...
											<c:if test="${detail.isDefault ne 1 }">
												<c:if test="${CommonOwnerEditAuth eq 0}"></c:if>
				                                <c:if test="${CommonOwnerEditAuth eq 1}"><a data-authority="base_custEdit" href='###' id="del_${detail.tscidId }" class='lk'>删除</a></c:if>				                               	
