...
<head>
    ...
    <script type="text/javascript">
        function update() {
            ...
            var roleCode = $("#update-roleCode").val();
            var roleName = $("#update-roleName").val();
            var roleDesc = $("#update-roleDesc").val();
            var postData = {
                roleId: roleId,
                roleCode: roleCode,
                roleDesc: roleDesc,
            ...
            $.post(ctx + "/role/update", postData, function (msg) {
                if (msg == 0) {
                    parent.addTabFun({
                        src: ctx + "/role/index",
                        title: "角色管理"
                    });
                    parent.closeTab("修改角色");
                } else if (msg == -1) {
...
<div class="panel-header">
    <div class="panel-title">修改角色【${item.roleName}】</div>
    <div class="panel-tool"></div>
</div>
<div style="margin: 10px;">
    <form id="update-form" class="formc" action="#">
        <input type="hidden" id="update-roleId" value="${item.roleId }"/>
        <table>
            <table>
                <tr height="30px"></tr>
                <tr>
                    ...
                    <td><input type="text" id="update-resourceName" class="easyui-textbox"
                               data-options="required:true,missingMessage:'角色名称不能为空!',prompt:'请输入角色名称...'"
                               style="width:200px;" maxlength="200" validtype="length[1,200]"
                               invalidMessage="有效长度为1-200" value="${item.roleName}"/>
                    </td>
                </tr>
                <tr height="8px"></tr>
                <tr valign="top">
                    ...
                    <td>
                        <input id="update-roleCode" class="easyui-textbox"
                               data-options="multiline:false,prompt:'请输入地址...'"
                               style="width: 450px;" maxlength="200" validtype="length[1,200]"
                               invalidMessage="有效长度为1-200" value="${item.roleCode}">
                    </td>
                <tr height="8px"></tr>
                <tr valign="top">
                    <td align="right">角色描述：</td>
                    <td>
                        <input id="update-roleDesc" class="easyui-textbox"
                               data-options="multiline:false,prompt:'请输入地址...'"
                               style="width: 450px;" maxlength="200" validtype="length[1,200]"
                               invalidMessage="有效长度为1-200" value="${item.roleDesc}">
                    </td>
                <tr height="8px"></tr>
            </table>
