...
<script type="text/javascript" src="${ctx}/static/js/view/res/myIntCustsData.js${_v}"></script>
...
<body>
	...
	<form id="queryForm" action="${ctx }/res/cust/myIntCusts" method="post">
	<!-- 加入计划字段 -->
		...
		<div class="hyx-cma hyx-layout">
			<div class="hyx-cm-left hyx-layout-content">
				<div class="com-search hyx-cm-search">
					<div class="com-search-box fl_l">
					   ...
					   <div class="select_outerDiv fl_l">
						   ...
						   <dl class="select resGroup pos${empty mutilSearchCodeSortMap['groupId'] ? '0' : mutilSearchCodeSortMap['groupId']}" data-input="[name='groupId']">
                            ...
                            <!-- class pos1 到 pos10 就是十个位置显示 ，后端根据数据处理位置-->
						   ...
						   <!-- 销售进程 -->
							...
							<c:set var="saleProcessName" value="" />
							<dl class="select pos${empty mutilSearchCodeSortMap['saleProcessId'] ? '0' : mutilSearchCodeSortMap['saleProcessId']}" data-input="[name='saleProcessId']">
								<c:if test="${empty saleProcessId}">
									<dt>销售进程</dt>
								</c:if>
								<c:if test="${not empty saleProcessId}">
									<c:forEach items="${options }" var="os">
										<c:if test="${saleProcessId eq os.optionlistId}">
										<dt>${os.optionName }</dt>
										</c:if>
									</c:forEach>
								</c:if>
								<dd>
									<ul>
										<c:if test="${empty saleProcessId}">
											<li><a href="javascript:void(0);" data-value="${saleProcessId}" title="销售进程">销售进程</a></li>
											<c:forEach items="${options }" var="os">
												<li><a href="javascript:void(0);"  data-value="${os.optionlistId}" title="${os.optionName }">${os.optionName }</a></li>
											</c:forEach>
										</c:if>
										<c:if test="${not empty saleProcessId}">
											<c:forEach items="${options }" var="os">
								...
								<input type="hidden" name="saleProcessId" id="s_saleProcessId" value="${saleProcessId}" />
							</dl>
						   <c:choose>
							...
							<dl class="select pos${empty mutilSearchCodeSortMap['startActionDate'] ? '0' : mutilSearchCodeSortMap['startActionDate']}" data-input="[name='lDateType']">
								<!-- 最近联系时间 -->
						   ...
						   <dl class="select pos${empty mutilSearchCodeSortMap['pstartDate'] ? '0' : mutilSearchCodeSortMap['pstartDate']}">
							...
							<c:if test="${commonOnwerOpen eq 1 }">
								<div class="reso-sub-dep fl_l pos${empty mutilSearchCodeSortMap['commonAccsStr'] ? '0' : mutilSearchCodeSortMap['commonAccsStr']}">
					...
					<input class="query-button fl_r" type="button" value="高级查询"  id="superQuery" data-url="${ctx }/highSearch/toPage?module=5" data-title="高级查询" data-hidden="true" data-param="&status=${status }" />
				...
				<p class="com-moTag" data-hidden-box="true">
					...
					<c:if test="${commonOnwerOpen  eq 1 }">
						<a href="###" nt="9" class="e">共有客户</a>
					</c:if>
					<c:if test="${will_recyle gt 0 }">
						<a href="###" nt="10" class="e">三日内回收</a>
					</c:if>
				</p>
				<p class="com-moTag" data-hidden-box="true">
				...
				<div class="com-btnlist hyx-cm-btnlist fl_l" data-hidden-box="true">
					...
					<a href="###" btn-type="auth" auth_id="base_putIntoHighSeas" id="giveUpBtn" class="com-btna demoBtn_a fl_l"><i class="min-turn-will"></i><label class="lab-mar">放入公海</label></a>
					...
					<c:if test="${commonOnwerOpen  eq 1 }">
						<a href="###" btn-type="auth" auth_id="myIntCust_setShare" id="addCommonOwner" class="com-btna demoBtn_a fl_l"><i class="min-new-add"></i><label class="lab-mar">设置共有者</label></a>
						<a href="###" btn-type="auth" auth_id="myIntCust_setShare" id="cancleCommonOwner" class="com-btna demoBtn_a fl_l"><i class="min-dele"></i><label class="lab-mar">取消共有者</label></a>
					</c:if>
				</div>
					...
					<table id="t1" width="100%" cellspacing="0" cellpadding="0" class="sty-borcolor-a sty-borcolor-c com-table-b com-radius ajax-table">
						<thead>
							<tr role="head" class="sty-bgcolor-b">
								...
								<th hidesort="true"><span class="sp sty-borcolor-b" sortColumn="AMOYTOCUSTOMER_DATE" sort="true">淘到客户<!-- <span class="td_sort_asc"></span> --></span></th>
