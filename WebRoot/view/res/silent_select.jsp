						...
						<c:choose>
							<c:when test="${!empty custs }">
								<c:forEach items="${custs }" var="cust">
									<tr>
										<td style="width:30px;"><div class="overflow_hidden w30 skin-minimal"><input type="checkbox" owner="${cust.ownerAcc }" id="chk_${cust.resCustId }" /></div></td>
										...
										<td><div class='overflow_hidden w120' title='<fmt:formatDate value="${cust.contractEndDate }" pattern="yyyy-MM-dd"/>'><fmt:formatDate value="${cust.contractEndDate }" pattern="yyyy-MM-dd"/></div></td>
