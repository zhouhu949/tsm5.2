...
/*             if (cust_check_tel2 == '1') {
        ...
        //设置显示或隐藏button （当值为空，或者有安全设置时，复制按钮隐藏）
...
<form id="resForm" action="${ctx }/res/cust/editRes" method="post">
    <div class='bomp-cen bomp_change <c:if test="${dayPlanflag eq 1 }">daily-planedit-cust-box</c:if>'>
                        ...
                        <label class='lab_a fl_l'>
                            <c:choose>
                                ...
                                ${field.fieldName}：
                                       ...
                                       <c:if test="${field.isRead eq 0 }">onclick="WdatePicker({maxDate:'%y-%M-%d'})"</c:if> class='ipt_a fl_l'/>
                        <span class="error" name='a' id="error_${ field.fieldCode}"></span>
                    </p>
                </c:when>
                        ...
                        <label class='lab_a fl_l'>
                            <c:choose>
                                ...
                                ${field.fieldName}：
                                       ...
                                       checkProp="${field.isRequired eq 0?'':'chk_1_1' }"  ${(field.isRead eq 0  || isRead) ? '':'readonly'}
                                       class='ipt_a fl_l'/>
                        <span class="error" name='a' id="error_${ field.fieldCode}"></span>
                    </p>
                </c:when>
