...
<script type="text/javascript" src="${ctx}/static/js/view/res/importRes.js${_v}"></script>
...
<div class="cust-impo-reso">
	...
	<div class="cust-impo-res" id="role-menu1">
		...
		<%-- <div class="com-search clearfix">
			<label class="fl_l" for="" style="width:auto;height:27px;margin-top:3px;">分组：</label>
			<dl class="select" style="z-index: 1;">
				<c:choose>
				<c:when test="${resGroupId!=null}">
					<c:forEach items="${resGroups}" var="group">
						<c:if test="${resGroupId==group.resGroupId }"><dt>${group.groupName}</dt></c:if>
					</c:forEach>
				</c:when>
				...
				<dd>
					<ul>
						<li><a onclick="javascript:setGroupId(null)" title="--请选择--">--请选择--</a></li>
					<c:forEach items="${resGroups}" var="group">
						<li><a onclick="javascript:setGroupId('${group.resGroupId}')" title="${group.resGroupId}">${group.groupName}</a></li>
					</c:forEach>
					</ul>
				</dd>
			</dl>
