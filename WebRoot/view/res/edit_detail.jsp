    ...
    <script type="text/javascript">
        $(function () {
            ...
            window.onload = function () {
                var height = $(".bomp-mcl").height() + 30;
                window.parent.$("#iframepage_a").css({"height": height + "px"});
            };
            /*表单优化*/
            ...
            $('#saveBtn').click(function () {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                    	var custId = $('#rciId').val();
                    	retUrl(custId,data,custId);
                    },
                    error: function (data) {
                    }
                });
            ...
            $('#cancelBtn').click(function () {
				...
				location.href = ctx + '/res/cust/toEditRes?custId=' + $("#rciId").val() + '&v=' + getTimestamp+'&dayPlanflag=1';
                </c:when>
                <c:otherwise>
                location.href = ctx + '/res/cust/toEditRes?custId=' + $("#rciId").val() + '&v=' + getTimestamp;
                </c:otherwise>
                </c:choose> 
		...
		function retUrl(custId,data,oldCustId){
             var getTimestamp = new Date().getTime();
             if (data == 1) {
                ...
                // location.href = ctx + '/res/cust/toEditRes?custId=' + custId + '&v=' + getTimestamp;
                 setTimeout('closeParentPubDivDialogIframe("edit_'+oldCustId+'");', 1000);
             } else {
                 window.top.iDialogMsg("提示", "保存失败！");
                  <c:choose>
                <c:when test="${dayPlanflag eq 1 }">
                location.href = ctx + '/res/cust/toEditRes?custId=' + custId + '&v=' + getTimestamp+'&dayPlanflag=1';
                </c:when>
                <c:otherwise>
                location.href = ctx + '/res/cust/toEditRes?custId=' + custId + '&v=' + getTimestamp;
                </c:otherwise>
...
<form action="${ctx}/res/cust/saveDetail" method="post" id="myForm">
    <div class='bomp-cen bomp_change bomp-mcl  project-comp-idialog-box <c:if test="${dayPlanflag eq 1 }">daily-planedit-cust-box</c:if>'>
        ...
        <input type="hidden" name='idReplaceWord' id='idReplaceWord' value='${idReplaceWord}'>
            <div class="check fl_r"><input type='checkbox' ${detail.isDefault eq 1 ? 'checked':''}
                                           readonly="${detail.isDefault eq 1 ?'readonly': ''}" name="mainLink"
                                           class='fl_l' id="mainLink"/><label class='fl_l font-s'>主要联系人</label></div>
        </div>
                    ...
                    <c:when test="${'telphone' eq field.fieldCode && field.enable eq 1  }">
                        <p class='bomp-p p_relative' id="bomp_error_${field.fieldCode }">
                            ...
                            <input type="hidden" id="${field.fieldCode }" name="${field.fieldCode }"
                                   ...
                                   class='ipt_a fl_l'/>
                            <span class="copy_telno" title="复制" onclick="copyPhone('telphone_safe')"
                    ...
                    <c:when test="${'telphonebak' eq field.fieldCode && field.enable  eq 1  }">
                        <p class='bomp-p p_relative' id="bomp_error_${field.fieldCode }">
                            ...
                            <input type='hidden' id="${field.fieldCode }" name="${field.fieldCode }"
                                   ...
                                   checkDetailProp="${field.isRequired eq 0?'chk_1_0':'chk_1_1' }"       ${(field.isRead eq 0  || isRead)  ? '':'disabled'}
                                   class='ipt_a fl_l'/>
                            <span class="copy_telno" title="复制" id="button_telphonebak_copy" onclick="copyPhone('telphonebak_safe')"
                                  style="${(idReplaceWord eq '1' or empty detail.telphonebak  )?'display: none':'display:block'}" ></span>
                            ...
                            <input type='text' id="${field.fieldCode }_safe" name="${field.fieldCode }_safe"
                                   value='${detail.telphonebak }'
                                   checkDetailProp="${field.isRequired eq 0?'chk_1_0':'chk_1_1' }"       ${(field.isRead eq 0  || isRead)  ? '':'disabled'}
                                   class='ipt_a fl_l' onblur="hiddenOrShowButton('telphonebak','${idReplaceWord}')"/>
                            <span class="error" id="error_${ field.fieldCode}"></span>
                        </p>
                    </c:when>
			    	   ...
			    	   <c:choose>
			            ...
			            <c:when test="${ field.dataType eq 2 }">
								<p class='bomp-p'>
								   	...
								   	<input type='text'  onclick="WdatePicker()"  id="${field.fieldCode }"  name="${field.fieldCode }"  value='${field.showValue }'  checkProp="${field.isRequired eq 0?'':'chk_5_1' }" class='ipt_a w_b fl_l' <c:if test="${field.isRead eq 1 }">readonly</c:if> />
									<span class="error"  name='a' id="error_${ field.fieldCode}"></span>
								</p>		                                    
			            </c:when>
        ...
        <div class='bomb-btn project-comp-idialog-btnbox' style="margin-bottom:10px;">
