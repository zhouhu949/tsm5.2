...
<head>
	...
	<script type="text/javascript" src="${ctx}/static/js/view/callreccord/callRecordList.js${_v}"></script>
</head>
<body>
<div class="hyx-ctr" >
      ...
      <div class="search clearfix" >
		   ...
		   <span class="icon_down_5px"></span>
		    <select class="fl_l search_head_select"  name="queryType">
	   		<option value = "1"  <c:if test="${queryType eq 1}">selected</c:if>>客户名称</option>
	   		<option value = "2"  <c:if test="${queryType ne 1 }">selected</c:if>>联系号码</option>
	   		</select>
		   	</div>
	   ...
	   <div class="list-box">
		   ...
		   <dl class="select">
			   ...
			   <c:choose>
			   		...
			   		<c:when test="${dDateType eq 5 }">
			   				<fmt:parseDate var="nsd"  value="${startDate }" pattern="yyyy-MM-dd HH:mm:ss" />
						<fmt:parseDate var="ned"  value="${endDate }"  pattern="yyyy-MM-dd HH:mm:ss" />
			   			<c:set var="dDateName">
		   ...
		   <input type="hidden" name="saleProcessId" id="saleProcessId" value="${saleProcessId }" />
				...
				<dl class="select">
					<dt style="border-right:0 none !important;">${nowsn }</dt>
					<dd>
						<ul>
		   ...
		   <p class='tag fl_l' style="position:relative;width:133px;padding:5px 0px 5px 4px;"><label class="fl_l">通话时长：</label>
				<input class="fl_l" type="text" id="timeLength" name="timeLengthBegin"   onblur="this.value=this.value.replace(/\D/g, '');" style="width:24px;margin:0 3px;" placeholder="秒">
				<span class="fl_l">-</span>
				<input class="fl_l" type="text" id="timeLengthEnd" name="timeLengthEnd"  onblur="this.value=this.value.replace(/\D/g, '');" style="width:24px;margin:0 3px;" placeholder="秒">
			</p>

			<div class="reso-sub-dep fl_l pos5">
						<c:choose>
							<c:when test="${shiroUser.issys eq 0}">
								<input class="owner-sour-nostr" name="accs"  readonly="readonly"  type="text"  value="${shiroUser.name }" >
							</c:when>
							<c:otherwise>
								<input name="accs" id="accs" type="hidden" value="${not empty accs ? accs : shiroUser.account}">
		...
		<a href="${ctx }/callrecord/historyList" class="ex-button"  data-authority="callRecord_checkHistory" >查看历史</a>
   </div>
   <p class="com-moTag">
    	 <!-- 呼叫类型 -->
		<input type="hidden" name="callState_"  id="callState"  value="${callState}">
		<label class="a">呼叫类型：</label>
		<a id="i_" class="e e-hover"  href="javascript:iCallType('');">全部</a>
		<a id="i_1" class="e"  href="javascript:iCallType('1');">已接来电</a>
		<a id="i_2" class="e" href="javascript:iCallType('2');">已接去电</a>
		<a id="i_3" class="e" href="javascript:iCallType('3');">未接来电</a>
		<a id="i_4" class="e" href="javascript:iCallType('4');">未接去电</a>
	</p>
	 <div class="com-btnlist hyx-cm-btnlist play-allsound-box">
	       <a href="###"  data-authority = 'callRecord_recordPlay' id="play-all-soundlist" class="com-btna reso-manag-f fl_l" title="播放当前页录音"><label>播放全部录音</label></a>
	       <!-- <a href="###"  data-authority = 'callRecord_recordDownload' id="download-soundlist" class="com-btna reso-manag-f fl_l" title="下载当前页录音"><label>下载全部录音</label></a> -->
	</div>
		...
		<table width="100%" cellspacing="0" cellpadding="0" class="sty-borcolor-a sty-borcolor-c com-table-b com-radius ajax-table">
			<thead>
				<tr class="sty-bgcolor-b">
							...
							<th><span class="sp sty-borcolor-b"><label class="lab-d"><span>联系人</th>
							<th>通话来源</th>
						</tr>
					</thead>
					<tbody>
	...
	<p class="reminder"><span style="color:red;">温馨提示：</span>用户可查询近3个月的通话记录</p>
</div>
</body>
</html>
