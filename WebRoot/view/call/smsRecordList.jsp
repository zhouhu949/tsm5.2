...
<title>通信管理-短信发送记录</title>
</head>
<body>
<div class="hyx-ctr" >
<form id="myForm" action="${ctx}/sms/record/smsSendList.do" method="post">
   <div class="com-search hyx-cfu-search">
      <div class="com-search-box fl_l">
	   <p class="search clearfix"><input class="input-query fl_l" type="text" name="smsComment" value="${item.smsComment }"/><label class="hide-span">输入短信内容</label></p>
	   <div class="list-box">
    ...
    <div class="com-btnlist hyx-cm-btnlist" style="margin:0 auto;margin-top:37px;">
					...
					<c:when test="${not empty list }">
						<c:forEach items="${list }" var="sms" varStatus="vs">
							<tr class=" ${vs.count%2==0?'sty-bgcolor-b':''}">
								<td style="width:30px;"><div class="overflow_hidden w30 skin-minimal"><input type="checkbox" name="smsId_ckb" value="${sms.smsId }" id="chk_${sms.smsId }" /></div></td>
								<td style="width:50px;"><div class="overflow_hidden w50 link"><a href="###" onclick="sendDetail('${sms.smsId }')"  class="icon-detail" title="查看明细" style="margin-left:13px;"></a></div></td>
								...
								<td><div class="overflow_hidden w70" >${sms.smsNumber }</div></td>
								<td><div class="overflow_hidden_01 w450" style="text-align:left !important;" title="${sms.smsComment}${sms.signature}">${sms.smsComment}${sms.signature}</div></td>
...
<script type="text/x-handlebars-template" id="template">
  {{#each list}}
	<tr class="{{even_odd @index}}">
		<td style="width:30px;"><div class="overflow_hidden w30 skin-minimal"><input type="checkbox" name="check_" value="{{smsId}}" /></div></td>
		<td style="width:50px;"><div class="overflow_hidden w50 link"><a href="javascript:;" onclick="sendDetail('{{smsId }}')"  class="icon-detail" title="查看明细" style="margin-left:13px;"></a></div></td>
		<td><div class="overflow_hidden w150 link" title="{{formatDate sendDate 'YYYY-MM-DD'}}">{{formatDate sendDate 'YYYY-MM-DD'}}</div></td>
		<td><div class="overflow_hidden w70" >{{smsNumber }}</div></td>
		<td><div class="overflow_hidden_01 w450" style="text-align:left !important;" title="{{smsComment}}{{signature}}">{{smsComment}}{{signature}}</div></td>
		<td><div class="overflow_hidden w120" title="{{ownerName }}">{{ownerName }}</div></td>
    </tr>
 {{/each}}
</script>
</html>
