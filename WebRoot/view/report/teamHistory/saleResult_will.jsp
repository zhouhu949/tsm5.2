...
<title>销售统计-小组历史数据-销售结果统计-意向</title>
<!--本页面样式-->
...
$(function(){
	...
	$("#clear").click(function(){
	 ...
	 $(".groupData").live("click",function(){
	 		...
	 		$("#iframepage_2").attr("src","${ctx}/report/teamHistoryData/saleResultUser?pageType=will&groupId="+groupId+"&fromDateStr=${fromDateStr}&toDateStr=${toDateStr}");
	 });
...
<div class="dataData_group">
	<div class="bomp_ic_tip">
		<ul class="list">
			<li _url="${ctx}/report/teamHistoryData/saleResult?pageType=res" >资源联系结果</li>
			<li class="li_click" _url="${ctx}/report/teamHistoryData/saleResult?pageType=will">意向联系结果</li>
		</ul>
	</div>
		...
		<table width="100%" cellspacing="0" cellpadding="0" class="sty-borcolor-a sty-borcolor-c com-table-a">
			...
			<tbody>
				...
				<c:when test="${!empty list}">
						...
						<td><div class="overflow_hidden w70" title="${bean.willSignNum }">
								 <c:choose>
				                	<c:when test="${bean.willSignNum > 0 }">
				                		<a class="view_change" href="javascript:;" groupId="${bean.groupId }" cust_type="2" change_type="2">${bean.willSignNum }</a>
				                	</c:when>
				                ...
				                <td><div class="overflow_hidden w70" title="${bean.willCustNum }">
								 <c:choose>
				                	<c:when test="${bean.willCustNum > 0 }">
				                		<a class="view_change" href="javascript:;" groupId="${bean.groupId }" cust_type="2" change_type="1">${bean.willCustNum }</a>
				                	</c:when>
					...
					<%-- <tr class="${list.size()%2!=0?'sty-bgcolor-b':''}">
						<td><div class="overflow_hidden w120" title="${total.groupName }">${total.groupName }</div></td>
						<td><div class="overflow_hidden w70" title="${total.willPlanNum }">${total.willPlanNum }</div></td>
						<td><div class="overflow_hidden w70" title="${total.willTotalNum }">${total.willTotalNum }</div></td>
						<td><div class="overflow_hidden w70" title="${total.willSignNum }">${total.willSignNum }</div></td>
						<td><div class="overflow_hidden w70" title="${total.willCustNum }">${total.willCustNum }</div></td>
						<td><div class="overflow_hidden w70" title="${total.willPoolNum }">${total.willPoolNum }</div></td>
						<td><div class="overflow_hidden w70" title="${total.willNoContact }">${total.willNoContact }</div></td>
						<td><div class="overflow_hidden w70" title="${total.willNoNum }">${total.willNoNum }</div></td>
					</tr> --%>
				</c:when>
				<c:otherwise>
	...
	<c:if test="${!empty list }">
		<iframe src="${ctx}/report/teamHistoryData/saleResultUser?pageType=will&groupId=${list[0].groupId}&fromDateStr=${fromDateStr}&toDateStr=${toDateStr}" width="100%" height="100%" id="iframepage_2" frameborder="0"
