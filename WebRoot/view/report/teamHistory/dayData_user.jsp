...
/* 	alert(	"window.parent"+"------"+window.parent.$("#iframepage_2").height());
	...
	$("#doExport").click(function(){
		...
		var type = 0;//日分析为0 月分析为1
		var urls = ctx+"/report/teamHistoryData/exportDataUser?fromDateStr="+fromDateStr +"&&toDateStr="+toDateStr +"&&groupId="+groupId +"&&column="+column +"&&orderType="+orderType+"&&type="+type ;
...
<form id="form" action="${ctx}/report/teamHistoryData/dayDataUser" method="post">
		...
		<table width="100%" cellspacing="0" cellpadding="0" class="sty-borcolor-a sty-borcolor-c com-table-a">
			<thead>
				<tr class="sty-bgcolor-b">
					...
					<th>回款金额（万元）<span sort="true" column="sign_money" class="${((column eq 'sign_money') && (orderType eq 'DESC')) ? 'td_sort_desc' :  'td_sort_asc'}"></th>
			                <th>签单金额（万元）</th>
			                <th>预期签单金额（万元）</th>
				</tr>
			</thead>
