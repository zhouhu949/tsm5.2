...
$(function(){
	$(".detail").live("click",function(){
		...
		window.parent.pubDivShowIframe("detail","${ctx }/report/teamHistoryData/saleResultDetail?pageType=res&userId="+userId+"&fromDateStr=${fromDateStr}&toDateStr=${toDateStr}",userName+"销售结果明细",800,400);
	 });
				...
				<tr class="${i.index%2!=0?'sty-bgcolor-b':''}">
					...
					<c:choose>
			                	<c:when test="${bean.resSignNum > 0 }">
			                		<a class="view_change" href="javascript:;" account="${bean.account }" cust_type="1" change_type="2">${bean.resSignNum }</a>
			                	</c:when>
							...
							<c:choose>
			                	<c:when test="${bean.resCustNum > 0 }">
			                		<a class="view_change" href="javascript:;" account="${bean.account }" cust_type="1" change_type="1">${bean.resCustNum }</a>
			                	</c:when>
