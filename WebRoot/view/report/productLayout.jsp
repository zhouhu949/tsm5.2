...
<head>
    ...
    <title>销售统计-团队客户分布-产品类型分布详情</title>
	<style type="text/css">
	...
	<script type="text/javascript">
		window.onload=function(){
			...
			if(firstGroup.length > 0){
				...
				var productIds = $("#productIds").attr("searchValue");
				loadAccountLayout(groupId,productIds);
			}
		};
		...
		function loadAccountLayout(groupId,productIds){
			$.post(ctx+"/layout/team/productReport/member",{groupId:groupId,productIds:productIds},function(data){
				$("#userDiv").html(data);
				$.parser.parse();
...
<body>
<form action="${ctx }/layout/team/productReport" method="post">
<!-- new -->
<div class="person-static-max">
	<div class="hyx-silent-p clearfix" style="z-index:555;">
		...
		<div class="com-search-report fl_l" style="margin-top:0;min-height:0">
			<label class="fl_l" for="" >选择产品：</label>
			<input type="hidden" id="productIds" searchValue="${productIds }" value="${productIds }" name="productIds" />
			<dl class="select fl_l" style="position:relative;z-index:555;width:145px;margin:0 auto;margin-right:10px;">
				...
				<dd>
					<ul id="tt2" class="easyui-tree" data-options="animate:true,dnd:false" style="max-height:160px;margin-top:10px;">
						<c:forEach items="${products }" var="product" varStatus="vs">
							<li><span><input name="col_checkbox" type="checkbox" ${empty colMap[product.id] ? '' : 'checked' } value="${product.id }">${product.name }</span></li>
						</c:forEach>
					</ul>
					<div class="sure-cancle clearfix" style="width:120px;margin:15px auto 0;padding-bottom:10px;">
		...
		<table width="100%" cellspacing="0" cellpadding="0" class="sty-borcolor-a sty-borcolor-c com-table-a">
			...
			<tbody>              
				<c:forEach items="${list2 }" var="pd" varStatus="vs">
					<tr <c:if test="${!empty pd.groupId }">id="view_group_${pd.groupId }"</c:if> class="${vs.count % 2 == 0 ? 'sty-bgcolor-b' : '' }">
	...
	<p class="hyx-mpe-ptit"><label class="com-red">温馨提示：</label>“/”前面的值表示该产品类型的数据量，“/”后面的值表示该产品类型的数量占总量的占比。</p>
</div>
