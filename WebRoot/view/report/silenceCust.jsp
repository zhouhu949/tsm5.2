    ...
    <script type="text/javascript">
        $(function () {
	       	 $('#queryId').click(function(){
					...
					$("#form").attr("action",ctx+"/report/silenceCust");
					$("#form").submit();
	     	 })
     	
        	 $('#exportId').click(function(){
				$("#form").attr("action",ctx+"/report/silenceCust/export");
				$("#form").submit();
        	})
        });
    	...
    	function toSilentDetail(currDate){
    		window.top.addTab(ctx+"/report/silenceCust/silentDetail?currDate="+currDate,"沉默客户统计(部门)");
    	}        
    </script>
</head>
<body>
<form id="form" action="${ ctx}/report/silenceCust" method="post">
               ...
               <c:when test="${ !empty list }">
                   <c:forEach items="${ list}" var="cust" varStatus="vs">
			            <tr class="${(vs.count % 2) ne 0 ? '' : 'sty-bgcolor-b' }">
			                ...
			                <td><div class="overflow_hidden w70" title="">${cust.silentTotal }</div></td>
			                <td><div class="overflow_hidden w70" title="10">${cust.addSilentTotal }</div></td>
			                <td><div class="overflow_hidden w70" title="">${cust.expireCustTotal }</div></td>
			                <td><div class="overflow_hidden w120" title="30%">${cust.silentRate }%</div></td>
				            <td>
				                <div class="overflow_hidden w80" title=""><a href="###" onclick="toSilentDetail('${cust.currDate}')">查看</a>
				                </div>
				            </td>			                
			            </tr>                   
                   </c:forEach>
