...
<html>
	...
	<body>
		<form data-render-url="${ctx }/newWill/OldDayNewWillSumByOption">
		...
		<script type="text/x-handlebars-template" id="template">
			...
			<tr class="{{even_odd @index}}">
				...
				{{/if}}
				<td>
					<div class="overflow_hidden w100" title="{{toDateInitType }}">{{toDateInitType }}</div>
				</td>
				<td>
					<div class="overflow_hidden w50" title="意向-{{currProcessName}}">意向-{{currProcessName}}</div>
				</td>
				<td>
					<div class="overflow_hidden w50" title="{{groupName}}">{{groupName}}</div>
				</td>
				<td>
					<div class="overflow_hidden w50" title="{{ownerAcc}}">{{ownerAcc}}</div>
				</td>
			</tr>
			{{/each}}
		</script>
