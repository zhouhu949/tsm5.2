...
<body>
	...
	<form id="cform" action="${ctx }/contract/order/saveOrder" method="post">
		...
		<div class='bomp-cen bomp-oad bomp-nca' style="padding-bottom:0 !important;width:99%;">
			...
			<div class="bomp-box bomp-box-margin-top10">
				<p class='bomp-p'>
					<label class='lab_a fl_l'><label class="bomp-red">*</label>订单编号：</label><input type='text' checkPub="chk_1_1_10" maxlength="50" name="orderBean.code" value='' class='ipt_a fl_l' />
					<span class="error"></span>
				</p>
				<p class='bomp-p'>
					<label class='lab_a fl_l'><label class="bomp-red">*</label>交易日期：</label><input type='text' checkPub="chk_1_1_0" name="orderBean.tradeDate" onclick="WdatePicker({maxDate:'%y-%M-%d'})" value='' class='ipt_a bomp-date fl_l' id="d1" />
					<span class="error"></span>
				</p>
				<p class='bomp-p'>
					<label class='lab_a fl_l'><label class="bomp-red">*</label>付款方式：</label>
					<select name="orderBean.payType" checkPub="chk_2_1_0" class='sel_a fl_l'>
						<option value="">请选择</option>
	                    <option value="1">现金</option>
	                    <option value="2">银行转账/汇款</option>
	                    <option value="3">在线支付</option>
	                    <option value="4">支付宝转账</option>
					</select>
					<span class="error"></span>
				</p>
			</div>
			<div class="bomp-box">
				<p class='bomp-p'>
					<input type="hidden" id="caid" name="orderBean.caId" value="${cDto.id }" />
				...
				<p class='bomp-p'>
					<label class='lab_a fl_l'>合同名称：</label><input id="caName" readonly="readonly" type='text' value='${cDto.contractName }' class='ipt_a fl_l' />
				</p>
				<p class='bomp-p'>
					<input type="hidden" name="orderBean.custId" value="${custInfoBean.resCustId }" />
					<label class='lab_a fl_l'>${empty filedMap['name'] ? '客户名称' : filedMap['name'] }：</label><input readonly="readonly" type='text' name="orderBean.custName" value='${custInfoBean.name }' class='ipt_a fl_l' />
				</p>
			</div>
			
			<div class="bomp-box">
				<p class='bomp-p'>
					<label class='lab_a fl_l'>生效日期：</label><input type='text' name="orderBean.effectiveDate" onclick="WdatePicker({maxDate:'#F{$dp.$D(\'d3\')}'})" value='' class='ipt_a bomp-date fl_l' id="d2" />
				</p>
				<p class='bomp-p'>
					<label class='lab_a fl_l'>失效日期：</label><input type='text' name="orderBean.invalidDate" onclick="WdatePicker({minDate:'#F{$dp.$D(\'d2\')||\'%y-%M-%d\'}'})" value='' class='ipt_a bomp-date fl_l' id="d3" />
				</p>
				<p class='bomp-p' style="width: 298px;">
					<label class='lab_a fl_l'>订单金额：</label><input type='text' id="totalMoney" checkPub="chk_1_0_3" readonly="readonly" name="orderBean.money" value='' class='ipt_a fl_l' /><span class="bomp-nca-comp fl_l">元</span>
					<span class="error"></span>
				</p>
			</div>
			<div class="bomp-box">
				<p class='bomp-p'>
					<c:choose>
						<c:when test="${shrioUser.isState eq 1 }">
							<label class='lab_a fl_l'>联系人：</label><input readonly="readonly" type='text' name="orderBean.linkName" value="${custInfoBean.mainLinkman }"  class='ipt_a fl_l'  />
						</c:when>
						<c:otherwise>
							<label class='lab_a fl_l'>单位名称：</label><input readonly="readonly" type='text' name="orderBean.company" value="${custInfoBean.company }"  class='ipt_a fl_l'  />
						</c:otherwise>
					</c:choose>
				</p>
				<p class='bomp-p'>
					<label class='lab_a fl_l'>联系电话：</label><input readonly="readonly" type='text' name="orderBean.linkPhone" value="${empty custInfoBean.mobilephone ? custInfoBean.telphone : custInfoBean.mobilephone }"  class='ipt_a fl_l'  />
				</p>
				<p class='bomp-p'>
					<label class='lab_a fl_l'>物流单号：</label><input type='text' maxlength="20" checkPub="chk_1_0_0" name="orderBean.courierNumber" value='' class='ipt_a fl_l' />
					<span class="error"></span>
				</p>
			</div>
			...
			<div class="bomp-box">
				<p class='bomp-p' style="width: 577px;">
					<label class='lab_a fl_l'>联系地址：</label><input readonly="readonly" type='text' name="orderBean.linkAddress" value="${shrioUser.isState eq 1 ? custInfoBean.address : custInfoBean.companyAddr }" class='ipt_a fl_l' style="width: 470px !important;" />
				</p>
				<p class='bomp-p'>
					...
					<select name="orderBean.salesOppId"  class='sel_a fl_l'>
	                    ...
	                    <c:forEach items="${saleChances }" var="saleChance">
	                    	<option value="${saleChance.saleChanceId }">${saleChance.saleChanceName }</option>
	                    </c:forEach>
					</select>
				</p>
			</div>
			...
			<div class="bomp-box">
				<p class='bomp-p bomp-oad-w'>
					<label class='lab_a fl_l'>订单说明：</label><textarea name="orderBean.context" maxlength="200" checkPub="chk_1_0_0" class='area_a fl_l'></textarea>
					<span class="error"></span>
				</p>
			</div>
			...
			<div class="com-table bomp_sps_table fl_l" style="width:96%;margin:15px 0 0 30px;">
				<input type="hidden" value="0" id="dlength" />
						...
						<tr id="pro_detail_0">
							<td>
								<div class="overflow_hidden">
									...
									<select id="pro_id_0" name="orderDetailBeans[0].productId" class="ipt_bor w80">
										<option value="">-请选择-</option>
										<c:forEach var="pro" items="${products }">
