...
<div class="year-sale-play">
    <ul class="ann-sale-plan clearfix">
        ...
        <li class=""  onclick="window.location.href='${ctx}/contract/order/list?noteType=${contractDto.noteType }';">订单维护</li>
        <li class="li_last" onclick="window.location.href='${ctx}/contract/orderinfo/list?noteType=${contractDto.noteType }';">订单详情</li>
    </ul>
</div>
<form action="${ctx }/contract/list" method="post" data-render-url="${ctx }/contract/data">
	                ...
	                <div class="list-box">
                      ...
                      <c:if test="${shiroUser.issys eq 1 }">
		                    <input type="hidden" name="ownerAccsStr" value="${contractDto.ownerAccsStr }" id="ownerAccsStr" />
      							...
      							<div class="reso-sub-dep fl_l">
      								<input id="ownerNameStr" readonly="readonly" class="owner-sour" type="text" value="所有者">
      								<div class="manage-owner-sour"  style="height:370px;">
	...
	<tr class="{{even_odd @index}}">
		<c:choose>
			<c:when test="${shiroUser.isState eq 1}">
				<td style="width:30px;">
					<div class="overflow_hidden w30 skin-minimal"><input name="check_" name_tel="{{mainLinkman }}|{{telphone}}" name_mobile="{{mainLinkman }}|{{mobilephone}}" mobile="{{mobilephone }}" telPhone="{{telphone}}" type="checkbox" id="chk_{{id }}"/></div>
				</td>
			</c:when>
			<c:otherwise>
				<td style="width:30px;">
					<div class="overflow_hidden w30 skin-minimal"><input name="check_" name_tel="{{custName }}|{{telphone}}" name_mobile="{{custName }}|{{mobilephone}}" mobile="{{mobilephone }}" telPhone="{{telphone}}" type="checkbox" id="chk_{{id }}"/></div>
				</td>
			</c:otherwise>
		</c:choose>
		<td style="width:130px;">
			<div class="overflow_hidden w130 link">
				{{contractOperateBtn ../shiroUser ../commonOnwerSign }}
			</div>
		</td>
		<td>
		...
		<td>
			<div class="overflow_hidden w50" title="{{formatContractType isDel}}">{{formatContractType isDel}}</div>
		</td>
		<td>
