...
<script type="text/javascript" src="${ctx }/static/js/view/qupai/loan-common.js${_v}"></script>
<script type="text/javascript" src="${ctx }/static/js/view/qupai/review-list.js${_v}"></script>
<style>
...
<body>
	<form method="post"  data-render-url="${ctx}/credit/review/findReviewInfoData"  id="unAssginForm">
		...
		<div class="hyx-dfpzy-reso hyx-layout">
			<div class="hyx-hsm-right hyx-layout-content" style="border: none;">
					...
					<div class="com-search-box fl_l">
					   ...
					   <div class="select_outerDiv fl_l">
						...
						<input type="hidden" name="auditStatus" id="auditStatus" value="">
			               <dl class="select pos${empty mutilSearchCodeSortMap['auditStatus'] ? '0' : mutilSearchCodeSortMap['auditStatus']}">
			                    <dt>审核状态</dt>
			                    <dd>
			                        <ul>
			                        	<li><a href="###" onclick="$('#auditStatus').val('');">审核状态</a></li>
			                            <li><a href="###" onclick="$('#auditStatus').val('1');">待确认</a></li>      
			                            <li><a href="###" onclick="$('#auditStatus').val('2');">已确认</a></li>    
			                            <li><a href="###" onclick="$('#auditStatus').val('3');">驳回</a></li>           
			                        </ul>
			                    </dd>
			                </dl>   
							<!-- 提交时间 -->
							<dl class="select pos${empty mutilSearchCodeSortMap['submitTime'] ? '0' : mutilSearchCodeSortMap['submitTime']}">
								<input type="hidden" class="startDate" name="submitStartDate" />
								<input type="hidden" class="endDate" name="submitEndDate" />
								<input type="hidden" class="type" name="submitDateType" />
								<dt>提交时间</dt>
								<dd>
									<ul>
						  ...
						  <!-- 借款日 -->
							<dl class="select pos${empty mutilSearchCodeSortMap['loanStartDate'] ? '0' : mutilSearchCodeSortMap['loanStartDate']}">
								<input type="hidden" class="startDate" name="loanStartDate" />
								<input type="hidden" class="endDate" name="loanEndDate" />
								<input type="hidden" class="type" name="loanDateType" />
								<dt>借款日</dt>
								<dd>
									<ul>
										<li><a href="###" onclick="setDateType(this,0)"
							...
							<!-- 到期日-->
							<dl class="select pos${empty mutilSearchCodeSortMap['dueStartDate'] ? '0' : mutilSearchCodeSortMap['dueStartDate']}">
								<input type="hidden" class="startDate" name="dueStartDate" />
								<input type="hidden" class="endDate" name="dueEndDate" />
								<input type="hidden" class="type" name="dueDateType" />
								<dt>到期日</dt>
								<dd>
									<ul>
										<li><a href="###" onclick="setDateType(this,0)"
							 ...
							 <div class="reso-sub-dep fl_l pos${empty mutilSearchCodeSortMap['ownerAccsStr'] ? '0' : mutilSearchCodeSortMap['ownerAccsStr']}">
									<c:choose>
										<c:when test="${shiroUser.issys eq 1 }">
											<input type="hidden" name="ownerAccsStr" class="tree-accs"  value="${item.ownerAccsStr}">
											<%-- <input type="hidden" name="comosType" class="tree-show-type" value="${item.comosType}" id="comosType" /> --%>
											<input class="owner-sour tree-show-value" id="comaccNames" data-placeholder="提交人"  type="text" value="提交人" style="border-right:2px solid #e1e1e1;">
										</c:when>
								 	</c:choose>
									<div class="manage-owner-sour" style="height:370px;" >
										...
										<div class="sub-dep-ul" style="width:190px;" data-type="search-tree">
											<ul id="founder-tree" class="member-tree" data-url="${ctx}/orgGroup/get_all_group_user_json">
										   </ul>
							    		</div>
								...
								<!-- 到期日-->
							<dl class="select pos${empty mutilSearchCodeSortMap['auditTime'] ? '0' : mutilSearchCodeSortMap['auditTime']}">
								<input type="hidden" class="startDate" name="auditStartDate" />
								<input type="hidden" class="endDate" name="auditEndDate" />
								<input type="hidden" class="type" name="auditDateType" />
								<dt>审核时间</dt>
								<dd>
									<ul>
										<li><a href="###" onclick="setDateType(this,0)"
				...
				<div class="com-btnlist hyx-cm-btnlist fl_l">
					 <button type="button"   id="output" class="fl_l btn btn-84px" ><i class="min-remove-resou icon"></i>导出</button>
				</div>
						...
						<table width="100%" cellspacing="0" cellpadding="0" id="t1"
							...
							<thead>
								<tr  role="head" class="sty-bgcolor-b">
									...
									<th><span class="sp sty-borcolor-b">提交时间</span></th>
	...
	<script type="text/x-handlebars-template" id="template">
	  {{#each list}}
		<tr class="{{even_odd @index}}">
			...
			<td><div class="overflow_hidden w40">{{leadReviewOperate leadId auditStatus}}</div></td>
			...
			<td><div class="overflow_hidden w70" title="{{auditStatus }}">{{auditStatusName }}</div></td>
			...
			<td><div class="overflow_hidden w70" title="{{ownerAcc }}">{{ownerName }}</div></td>
			<td><div class="overflow_hidden w100" title="{{formatDate submitTime 'YYYY-MM-DD'}}">{{formatDate submitTime 'YYYY-MM-DD'}}</div></td>
			<td><div class="overflow_hidden w100" title="{{formatDate auditTime 'YYYY-MM-DD'}}">{{formatDate auditTime 'YYYY-MM-DD'}}</div></td>
