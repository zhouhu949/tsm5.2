$(function(){
    $('.demoBtn_a').click(function(){
		pubDivShowIframe('historyWindow',ctx+'/plan/month/team/historyWindow?groupId='+$("#groupId").val(),'历史计划走势',915,400);
    });
// 点评文字省略
	$('.box').each(function(){
	...
	function overlay_show(button){
		...
		integral_change(button);
	}

	//遮罩层积分滚动
	function integral_change(button){
		var start_integral = 10;//初始积分
		var add_integral = 300;//增加积分
	...
	$('.receive').click(function(){//点击单个领取按钮
		...
		var _this=this;
		$.post(ctx+"/plan/month/team/receivePoint",{"id":$(_this).attr("planid"),"type":$(this).attr("pointtype"),"isLast":isLast},function(model){
	...
	$('.receive_all a').click(function(){//点击全部领取按钮
		var _this=this;
		$.post(ctx+"/plan/month/team/receivePoint",{"id":$(this).attr("planid"),"type":$(this).attr("pointtype")},function(model){
			if(model.status=="success"){
			$(_this).parent().hide();
