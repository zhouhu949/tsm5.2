$(function(){
	var checkBox= new CheckBox("checkAll","chk_","custId","state","resCustLevel");
	 ...
	 $("#join_cust").click(function(e){
		 ...
		 var saleprocessid = $this.data("saleprocessid");
		 var planid = $this.data("planid");
		 pubDivShowIframe('join_custs',ctx+'/res/cust/myIntCusts?planPage=1&planType=will&dateStr='+dateStr+'&&saleProcessId='+saleprocessid+'&planid='+planid,'加入客户',900,560);
	 });
	 $("#time_change").click(function(e){
		...
		if(checkBox.getIds()!=null){
			pubDivShowIframe('moveAnotherDayWindow',ctx+'/plan/day/moveAnotherDayWindow?type=will&sudId='+sudId+'&planDate='+planDate+'&custIds='+checkBox.custIds.toString(),'计划时间调整',450,330);
		}
	});
	 //标签点击
	 $(".com-moTag .e").click(function(e){
		 ...
		 var array=["全部","转签约","意向增进","转公海","意向无变化","已联系","未联系","重点客户"];
		//清除其他查询条件
		$("#queryText").val(null);
		$("#custLevel").val(null);
		$("#status").val(null);
...
function loadAjax(data,page){
	...
	$.ajax({
		url:ctx+"/plan/day/willcust/willDataJson",
		type:"get",
		data:data,
		cache:false,
