...
//			console.log(data_entitys);
	...
	$(".ajax-table>tbody").delegate(".is-query input[type='checkbox']","click",function(e){
		...
		var dataType8_length = $(".is-query input[data-type='8']:checked").length;
		if((checked_length-inputText_length-dataType8_length) < 1){
			e.preventDefault();
			warmDivDialog("warm","常用输入查询项数量最少为1个");
//			window.top.iDialogMsg("提示","常用输入查询项数量最少为1个");
			return false;
		}else if((checked_length-inputText_length-dataType8_length) > 10){
			e.preventDefault();
...
//		console.log(data_entitys);
		$.ajax({
			url: ctx + '/quPaiSearchManager/modfiy',
			type: "post",
...
function searchSetPage(value){
	window.location.href = ctx + '/quPaiSearchManager/searchSetPage?module='+value;
}
