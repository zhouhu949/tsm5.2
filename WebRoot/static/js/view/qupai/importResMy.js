	...
	if(data.length==headers.length){
		for(var i=0;i<data.length;i++){
			...
			for(var j=0;j<importFields.length;j++){
				html+='<option  '+(headerFiled[i]==importFields[j].fieldCode?'selected = "selected"':'')+'value="'+importFields[j].fieldCode+'">'+importFields[j].fieldName+'</option>';
			}			
			html+='</select></td>'+
			'	<td>'+data[i]+'</td>'+
			'</tr>';
	...
	if($arry.length>0){
		...
		if($("#firstCheck").is(":checked")){
		 ...
		 $.ajax({
	            type: "post",
	            dataType: "html",
	            url: tsmUpLoadServiceUrl+ctx + "/fileupload/leadimpHyx/matchFields",
...
$(function() {
	...
	$.ajax({
		...
		success: function(data){
			...
			$(".shows-third-select").html(appendCode);
		},
		error: function(){
