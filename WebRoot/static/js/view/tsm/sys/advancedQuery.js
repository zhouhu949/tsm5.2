...
//		_param+="&nDateType=1";
//		_param+="&page.totalResult=1&page.currentPage=1&page.showCount=20";
			parent.loaderAjax(_param);

			saveSelectedValues(data,module);
			closeParentPubDivDialogIframe("alert_high_search_e");
		});

		$(".sure-cancel #btn-cancel").on("click",function(e){
//		closeParentPubDivDialogIframe("alert_high_search_e");
			for(var i=0;i<data.length;i++){
				data[i].value = "";
				data[i].displayContent = "";
				var dataType = data[i].dataType;
				var childList = data[i].childList;
				if(childList && childList.length>0){
					for(var j=0; j<childList.length; j++){
						childList[j].isCheck = 0;
						if(dataType == "2"){
							childList[j].value = "";
						}
					}
...
//			tree.parents(".tree-box").find(".btn-submit").click();
	...
	function renderQueryContent(data) {
		...
		for(var i=0;i<data.length;i++){
			...
			var displayContent = !curData.displayContent?"":curData.displayContent;
			var appendCode = '<div class="advanced-query-block">';
			appendCode += '<div class="advancedQuery-everytitle">'+curData.searchName+'：<span class="data-show-area">'+displayContent+'</span></div>';
			switch (dataType) {
				case 2:
					...
					var isCheck_class0 = checkedValue=="0"?"active":"";
					var isCheck_checked0 = checkedValue=="0"?"checked='checked'":"";
					var isCheck_class1 = checkedValue=="1"?"active":"";
					var isCheck_checked1 = checkedValue=="1"?"checked='checked'":"";
					var isCheck_class2 = checkedValue=="2"?"active":"";
					var isCheck_checked2 = checkedValue=="2"?"checked='checked'":"";
					var isCheck_class3 = checkedValue=="3"?"active":"";
					var isCheck_checked3 = checkedValue=="3"?"checked='checked'":"";
					var isCheck_class4 = checkedValue=="4"?"active":"";
					var isCheck_checked4 = checkedValue=="4"?"checked='checked'":"";
					var isCheck_class5 = checkedValue=="5"?"active":"";
					var isCheck_checked5 = checkedValue=="5"?"checked='checked'":"";
...
//					appendCode += '<span class="advancedQuery-tagincont date-no-time '+isCheck_class0+'" title="无联系时间">无联系时间<input type="radio" name="radio_'+curData.searchCode+'" value="0" '+isCheck_checked0+'/></span>';
					appendCode += '<span class="advancedQuery-tagincont date-today '+isCheck_class1+'" title="当天">当天<input type="radio" name="radio_'+curData.searchCode+'" value="1" '+isCheck_checked1+'/></span>';
					appendCode += '<span class="advancedQuery-tagincont date-week '+isCheck_class2+'" title="本周">本周<input type="radio" name="radio_'+curData.searchCode+'" value="2" '+isCheck_checked2+'/></span>';
					appendCode += '<span class="advancedQuery-tagincont date-month '+isCheck_class3+'" title="本月">本月<input type="radio" name="radio_'+curData.searchCode+'" value="3" '+isCheck_checked3+'/></span>';
					if(developCode == "contractExpireTime" || developCode == "nextFollowTime"){
						appendCode += '<span class="advancedQuery-tagincont date-half-year-later '+isCheck_class4+'" title="半年">半年<input type="radio" name="radio_'+curData.searchCode+'" value="4" '+isCheck_checked4+'/></span>';
						appendCode += '<span class="advancedQuery-tagincont dateRange-later '+isCheck_class5+'" title="自定义">自定义<input type="radio" name="radio_'+curData.searchCode+'" value="5" '+isCheck_checked5+'/></span>';
					}else{
						appendCode += '<span class="advancedQuery-tagincont date-half-year-ago '+isCheck_class4+'" title="半年">半年<input type="radio" name="radio_'+curData.searchCode+'" value="4" '+isCheck_checked4+'/></span>';
						appendCode += '<span class="advancedQuery-tagincont dateRange '+isCheck_class5+'" title="自定义">自定义<input type="radio" name="radio_'+curData.searchCode+'" value="5" '+isCheck_checked5+'/></span>';
					}
					if(childList.length != 0 && childList[0].value != "" && childList[0].name != ""){
						appendCode += '<span class="advancedQuery-text data-show-area">'+childList[0].value+'/'+childList[1].value+'</span>';
						appendCode += '<input type="hidden" name="'+childList[0].name+'" value="'+childList[0].value+'" class="startDate" />';
						appendCode += '<input type="hidden" name="'+childList[1].name+'" value="'+childList[1].value+'" class="endDate" />';
				...
				case 3:
					...
					for(var j=0;j<childList.length;j++){
						...
						var isCheck_class = curChildList.isCheck==1?"active":"";
						var isCheck_checked = curChildList.isCheck==1?"checked='checked'":"";
						//公海客户--放弃资源原因  需要传递的是文字，不是code值
						if(curData.searchCode == "reason"){
							appendCode += '<span class="advancedQuery-tagincont ' + isCheck_class + '" title="'+curChildList.name+'">'+curChildList.name+'<input type="radio" name="'+curData.searchCode+'" value="'+curChildList.name+'"'+isCheck_checked+' /></span>';
						}else{
							appendCode += '<span class="advancedQuery-tagincont ' + isCheck_class + '" title="'+curChildList.name+'">'+curChildList.name+'<input type="radio" name="'+curData.searchCode+'" value="'+curChildList.value+'"'+isCheck_checked+' /></span>';
						}
					}
					appendCode += '</div>';
					break;
				case 4:
					...
					for(var j=0;j<childList.length;j++){
						...
						var isCheck_class = curChildList.isCheck==1?"active":"";
						var isCheck_checked = curChildList.isCheck==1?"checked='checked'":"";
						appendCode += '<span class="advancedQuery-tagincont ' + isCheck_class + '" title="'+curChildList.name+'">'+curChildList.name+'<input type="checkbox" name="'+curData.searchCode+'" value="'+curChildList.value+'"'+isCheck_checked+' /></span>';
					}
					appendCode += '</div>';
					break;
				case 5:
					...
					appendCode = appendCode +
						'<input type="hidden" class="input-tree" name="'+curData.searchCode+'" value="'+curData.value+'" />'+
						'<div class="tree-ul-box">'+
							'<ul data-type="tree" data-url="'+curData.paramValue+'" data-value="'+curData.value+'" data-searchCode="'+curData.searchCode+'"></ul>'+
						'</div>'+
					...
					break;
				case 6:
					var tempValue = curData.value!=""?curData.value.split(","):["","",""];
					appendCode += '<input type="hidden" name="countyId" data-id="" data-name="" id="harea" value="'+tempValue[2]+'">';
					appendCode += '<input type="hidden" name="cityId" data-id="" data-name="" id="hproper" value="'+tempValue[1]+'">';
					appendCode += '<input type="hidden" name="provinceId" data-id="" data-name="" id="hcity" value="'+tempValue[0]+'">';
					appendCode += 	
				...
				case 7:
					appendCode += 	
						'<div class="advancedQuery-everycontent res-group-checked" data-searchCode="'+curData.searchCode+'" data-dataType="'+curData.dataType+'">'+
							...
							'<div class="tree-box">'+
								'<input type="hidden" class="input-tree" name="'+curData.searchCode+'" value="'+curData.value+'" />'+
								'<ul data-type="resource-tree" data-url="'+curData.paramValue+'" data-value="'+curData.value+'" data-searchCode="'+curData.searchCode+'"></ul>'+
							'</div>'+
						'</div>';
					break;
				default:
		...
		'<div class="sure-cancel button-area">'+
			...
			'<button type="button" class="btn btn-default btn-md" id="btn-cancel">清空</button>'+
		'</div>';
		outerBox.append(appendButton);
