...
function loaderAjax(_param,pageStr){
	...
	var _url = ctx+'/res/cust/myIntCustsData';
	searchingDataTip();
	...
	for(var i = 0;i<data.list.length;i++){
		...
		html+=
			"<tr id='rightView_"+AjaxQueryData.emptyTransfer(res.resCustId)+"' class='"+(i%2==0?'':'sty-bgcolor-b')+"'>" +
				"<td style='width:30px;'><div class='overflow_hidden w30 skin-minimal'><input type='checkbox' name='check_' commonAcc='"+AjaxQueryData.emptyTransfer(res.commonAcc)+"' status='2' owner='"+AjaxQueryData.emptyTransfer(res.ownerAcc)+"' name_tel='"+AjaxQueryData.emptyTransfer(data.isState == 1 ? res.mainLinkman : res.name)+"|"+AjaxQueryData.emptyTransfer(res.telphone)+"' name_mobile='"+AjaxQueryData.emptyTransfer(data.isState == 1 ? res.mainLinkman : res.name)+"|"+AjaxQueryData.emptyTransfer(res.mobilephone)+"' mobile='"+AjaxQueryData.emptyTransfer(res.mobilephone)+"' telPhone='"+AjaxQueryData.emptyTransfer(res.telphone)+"' id='chk_"+AjaxQueryData.emptyTransfer(res.resCustId)+"' /></div></td>" +
				...
				"<td><div class='overflow_hidden w70' title='"+AjaxQueryData.emptyTransfer(res.saleProcessName)+"'>"+AjaxQueryData.emptyTransfer(res.saleProcessName)+"</div></td>" +
				...
				"<td hidevalue='"+AjaxQueryData.emptyTransfer(res.startDate )+"'><div class='overflow_hidden w110' title='"+AjaxQueryData.emptyTransfer(res.startDate )+"'>"+AjaxQueryData.emptyTransfer(res.endDate )+"</div></td>" +
				...
				"<td><div class='overflow_hidden w80' phone='tel' title='"+AjaxQueryData.emptyTransfer(res.mobilephone||res.telphone )+"'>"+AjaxQueryData.emptyTransfer(res.mobilephone||res.telphone )+"</div></td>" +
...
function getResOperate(res,serverDay,loginAcc,signSetting,adminSignAuth,isState,signAuth){
	...
	if(serverDay > 0){
			if(window.top.shrioAuthObj("base_followCust")){
				html+=
					"<a href='javascript:;' id='follow_"+AjaxQueryData.emptyTransfer(res.resCustId)+"' custType='2' class='icon-follow-up' title='跟进'></a>";
			}
			html+=
			...
			if(window.top.shrioAuthObj("base_signCust")){
				if((loginAcc == res.ownerAcc || (loginAcc != res.commonAcc && adminSignAuth == 1) || (loginAcc == res.commonAcc && signAuth == 1))){
					html+=
						"<a href='javascript:;' module='2' id='sign_"+res.resCustId+"' signSetting='"+signSetting+"' class='icon-sign-cont' title='签约'></a>";
				}else{
					html+=
