...
function loaderAjax(_param,pageStr){
	...
	var _url = ctx+'/res/cust/mySilentCustsData';
//	var _param = $('#queryForm').serialize();
	searchingDataTip();
	...
	for(var i = 0;i<data.list.length;i++){
		...
		html+=
			"<tr id='rightView_"+AjaxQueryData.emptyTransfer(res.resCustId)+"' class='"+(i%2==0?'':'sty-bgcolor-b')+"'>" +
				"<td style='width:30px;'><div class='overflow_hidden w30 skin-minimal'><input type='checkbox' id='chk_"+AjaxQueryData.emptyTransfer(res.resCustId)+"' /></div></td>" +
				...
				"<td><div class='overflow_hidden w80' title='"+AjaxQueryData.emptyTransfer(res.days)+"'>"+AjaxQueryData.emptyTransfer(res.days)+"</div></td>" +
...
function getResOperate(res,serverDay,loginAcc,signSetting,adminSignAuth,isState){
	...
	if(serverDay > 0){
		if(window.top.shrioAuthObj('base_followCust')){
			html+=
				"<a href='javascript:;' id='follow_"+AjaxQueryData.emptyTransfer(res.resCustId)+"' custType='4' class='icon-follow-up' title='跟进'></a>" ;
		}
		html+=
