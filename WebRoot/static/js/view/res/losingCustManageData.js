...
function loaderAjax(_param,pageStr){
	...
	var _url = ctx+'/res/cust/losingCustsData';
	searchingDataTip();
...
function loadRestsList(data){
	...
	for(var i = 0;i<data.list.length;i++){
		...
		html+=
			...
			"<td style='width:30px;'><div class='overflow_hidden w30 skin-minimal'><input type='checkbox' status='5' name='check_' owner='"+AjaxQueryData.emptyTransfer(res.ownerAcc)+"' name_tel='"+AjaxQueryData.emptyTransfer(data.isState == 1 ? res.mainLinkman : res.name)+"|"+AjaxQueryData.emptyTransfer(res.telphone)+"' name_mobile='"+AjaxQueryData.emptyTransfer(data.isState == 1 ? res.mainLinkman : res.name)+"|"+AjaxQueryData.emptyTransfer(res.mobilephone)+"' mobile='"+AjaxQueryData.emptyTransfer(res.mobilephone)+"' telPhone='"+AjaxQueryData.emptyTransfer(res.telphone)+"' id='chk_"+AjaxQueryData.emptyTransfer(res.resCustId)+"' /></div></td>" +
				...
				html+=
				"<td><div class='overflow_hidden w70' title='"+AjaxQueryData.emptyTransfer(res.losingReason)+"'>"+AjaxQueryData.emptyTransfer(res.losingReason)+"</div></td>" +
				...
				"<td><div class='overflow_hidden w90' title='"+AjaxQueryData.emptyTransfer(res.days)+"'>"+AjaxQueryData.emptyTransfer(res.days)+"</div></td>" +
				...
				"<td><div class='overflow_hidden w80' title='"+AjaxQueryData.emptyTransfer(res.custTypeName)+"'>"+AjaxQueryData.emptyTransfer(res.custTypeName)+"</div></td>" +
				"<td><div class='overflow_hidden w80' title='"+AjaxQueryData.emptyTransfer(res.groupName)+"'>"+AjaxQueryData.emptyTransfer(res.groupName)+"</div></td>" +
				"<td><div class='overflow_hidden w100' title='"+AjaxQueryData.emptyTransfer(res.losingRemark)+"'>"+AjaxQueryData.emptyTransfer(res.losingRemark)+"</div></td>" +
...
function getResOperate(res,serverDay,loginAcc,signSetting,adminSignAuth,isState){
	...
	if(window.top.shrioAuthObj('base_followCust')){
		if(serverDay > 0){
			html+="<a href='javascript:;' id='follow_"+AjaxQueryData.emptyTransfer(res.resCustId)+"' custType='5' class='icon-follow-up' title='跟进'></a>";
		}else{
			html+="<a href='javascript:;' class='icon-follow-gray' title='跟进'></a>";
		}
	}
	html+="<a href='javascript:;' id='cardInfo_"+AjaxQueryData.emptyTransfer(res.resCustId)+"' custName='"+getCardTitle(res,isState)+"' class='icon-cust-card' title='客户卡片'></a>";;
	return html;
}
