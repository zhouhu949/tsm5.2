...
$(function(){
		...
		upload_url: ctx + '/fileupload/resimpHyx/upload',
...
//		button_window_mode: SWFUpload.WINDOW_MODE.TRANSPARENT,  // Flash剪辑的WMODE属性
		...
		/*$(".shows-next-tip-btn").click(function(){
			mark = $("#mark").find("option:selected").val();		
			var processId = $("#processId").find("option:selected").val();
			var postobj = { "resGroupId": resGroupId,"status":status,"mark":mark,"processId":processId};
			swfUpload.setPostParams(postobj);
...
/*	case SWFUpload.QUEUE_ERROR.UPLOAD_LIMIT_EXCEEDED:
		...
		break;	*/
	default:
		window.top.iDialogMsg("提示",'加载入队列出错.');
		break;
	}
...
function fileDialogComplete(numSelected,numQueued,numTotalInQueued) {
	if (queueErrorArray && queueErrorArray.length) {
		...
		mark = $("#mark").find("option:selected").val();		
		var processId = $("#processId").find("option:selected").val();
		var postobj = { "resGroupId": resGroupId,"status":2,"mark":mark,"processId":processId};
		swfUpload.setPostParams(postobj);
		swfUpload.startUpload();
	}
}
