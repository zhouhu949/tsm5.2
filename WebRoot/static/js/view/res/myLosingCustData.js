...
function loaderAjax(_param,pageStr){
	...
	var _url = ctx+'/res/cust/myLosingCustsData';
	//var _param = $('#queryForm').serialize();
	searchingDataTip();
...
function loadRestsList(data){
	...
	for(var i = 0;i<data.list.length;i++){
		...
		html+=
			...
			"<td style='width:30px;'><div class='overflow_hidden w30 skin-minimal'><input type='checkbox' name='check_' owner='"+AjaxQueryData.emptyTransfer(res.ownerAcc)+"' status='5' name_tel='"+AjaxQueryData.emptyTransfer(data.isState == 1 ? res.mainLinkman : res.name)+"|"+AjaxQueryData.emptyTransfer(res.telphone)+"' name_mobile='"+AjaxQueryData.emptyTransfer(data.isState == 1 ? res.mainLinkman : res.name)+"|"+AjaxQueryData.emptyTransfer(res.mobilephone)+"' mobile='"+AjaxQueryData.emptyTransfer(res.mobilephone)+"' telPhone='"+AjaxQueryData.emptyTransfer(res.telphone)+"' id='chk_"+AjaxQueryData.emptyTransfer(res.resCustId)+"' /></div></td>" +
...
function getResOperate(res,serverDay,loginAcc,signSetting,adminSignAuth,isState){
	...
	if(serverDay > 0){
		if(window.top.shrioAuthObj('base_followCust')){
			html+=
				"<a href='javascript:;' id='follow_"+AjaxQueryData.emptyTransfer(res.resCustId)+"' custType='5' class='icon-follow-up' title='跟进'></a>" ;
		}
		html+=
