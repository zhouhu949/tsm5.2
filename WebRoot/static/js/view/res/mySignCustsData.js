...
function loaderAjax(_param,pageStr){
	...
	var _url = ctx+'/res/cust/mySignCustsData';
	searchingDataTip();
	...
	for(var i = 0;i<data.list.length;i++){
		...
		if(res != null){
			html+=
				...
				"<td style='width:30px;'><div class='overflow_hidden w30 skin-minimal'><input type='checkbox' name='check_' commonAcc='"+AjaxQueryData.emptyTransfer(res.commonAcc)+"' owner='"+AjaxQueryData.emptyTransfer(res.ownerAcc)+"' name_tel='"+AjaxQueryData.emptyTransfer(data.isState == 1 ? res.mainLinkman : res.name)+"|"+AjaxQueryData.emptyTransfer(res.telphone)+"' name_mobile='"+AjaxQueryData.emptyTransfer(data.isState == 1 ? res.mainLinkman : res.name)+"|"+AjaxQueryData.emptyTransfer(res.mobilephone)+"' mobile='"+AjaxQueryData.emptyTransfer(res.mobilephone)+"' telPhone='"+AjaxQueryData.emptyTransfer(res.telphone)+"' id='chk_"+AjaxQueryData.emptyTransfer(res.resCustId)+"' /></div></td>" +
				"<td style='width:130px;'>" +
				"<div class='overflow_hidden w130 link'>" +
				getResOperate(res,data.serverDay,data.loginAcc,data.signSetting,data.adminSignAuth,data.isState,data.commonOnwerSign) +
				"</div>" +
				"</td>" ;
			if(data.isState == 0){
				html+=
			...
			html+=
				...
				"<td><div class='overflow_hidden w70' title='"+AjaxQueryData.milliFormat(res.orderCountMonty+'')+"'>"+AjaxQueryData.milliFormat(res.orderCountMonty+'')+"</div></td>" +
				"<td><div class='overflow_hidden w70' title='"+AjaxQueryData.milliFormat(res.totalOrderAmount+'')+"'>"+AjaxQueryData.milliFormat(res.totalOrderAmount+'')+"</div></td>" +
				...
				"<td hidevalue='"+AjaxQueryData.emptyTransfer(res.pstartDate )+"'><div class='overflow_hidden w110' title='"+AjaxQueryData.emptyTransfer(res.pstartDate )+"'>"+AjaxQueryData.emptyTransfer(res.pendDate )+"</div></td>" +
				"<td hidevalue='"+AjaxQueryData.emptyTransfer(res.startDate )+"'><div class='overflow_hidden w110' title='"+AjaxQueryData.emptyTransfer(res.startDate )+"'>"+AjaxQueryData.emptyTransfer(res.startDate )+"</div></td>" +
				"<td><div class='overflow_hidden w50' title='"+AjaxQueryData.emptyTransfer(res.commonOwnerName )+"'>"+AjaxQueryData.emptyTransfer(res.commonOwnerName )+"</div></td>"+ 
				"<td><div class='overflow_hidden w50' title='"+AjaxQueryData.emptyTransfer(res.custTypeName )+"'>"+AjaxQueryData.emptyTransfer(res.custTypeName )+"</div></td>" + 
				...
				"<td><div class='overflow_hidden w50' title='"+AjaxQueryData.emptyTransfer(res.serviceName )+"'>"+AjaxQueryData.emptyTransfer(res.serviceName )+"</div></td>" + 
...
/**生成操作列*/
function getResOperate(res,serverDay,loginAcc,signSetting,adminSignAuth,isState,signAuth){
	var mySignCust_addContract = $("#mySignCust_addContract").val() || "0";
	var mySignCust_addOrder = $("#mySignCust_addOrder").val() || "0";
	var mySignCust_cancelSign = $("#mySignCust_cancelSign").val() || "0";
	var html = "";
	if(serverDay > 0){
			if(window.top.shrioAuthObj("base_followCust")){	
			html+=
					"<a href='javascript:;' id='follow_"+AjaxQueryData.emptyTransfer(res.resCustId)+"' custType='3' class='icon-follow-up' title='跟进'></a>" ;
			}
			html+=
			...
			if(window.top.shrioAuthObj("mySignCust_addOrder")){
				if(loginAcc == res.commonAcc && signAuth != 1){
					html+="<a href='javascript:;' class='icon-new-order img-gray' title='新增订单'></a>";
				}else{
					html+="<a href='javascript:;' module='3' id='addOrder_"+AjaxQueryData.emptyTransfer(res.resCustId)+"' class='icon-new-order' title='新增订单'></a>";
				}
			}
			//新增合同
			if(window.top.shrioAuthObj("mySignCust_addContract")){
				if(loginAcc == res.commonAcc && signAuth != 1){
					html+="<a href='javascript:;' class='icon-sign-gray' title='新增合同'></a>";
				}else{
					html+="<a href='javascript:;' module='3' id='addContract_"+AjaxQueryData.emptyTransfer(res.resCustId)+"' class='icon-new-contr' title='新增合同'></a>";
				}
			}
			//取消签约
			if(window.top.shrioAuthObj("mySignCust_cancelSign")){
				if(loginAcc == res.ownerAcc || (loginAcc != res.commonAcc && adminSignAuth == 1) || (loginAcc == res.commonAcc && signAuth == 1)){
					html+="<a href='javascript:;' id='unsign_"+AjaxQueryData.emptyTransfer(res.resCustId)+"' class='icon-cancel-contr shows-cancel-change' title='取消签约'></a>";
				}else{
					html+="<a href='javascript:;' class='icon-cancel-contr icon-cancel-change-gray'  title='取消签约'></a>";
				}
			}
	}else{
			if(window.top.shrioAuthObj("base_followCust")){
