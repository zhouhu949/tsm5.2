...
function chkInput(chkTag,chkRule){
	...
	// 超出最大长度
	if(chkVal!='' && maxLen!='' && !chkInputLen(chkVal,maxLen)){
...
function chkInputLen(chkVal,len){
	var pattern = /[^\x00-\xff]/g;
	return chkVal.replace(pattern, "g").length <= len;
}
...
function setErrMsg($tag,msg,bol){
	...
	var tag = $tag;
	if(msg !=null && msg !=''){
	    //修正不按 ：last 处理。不靠谱，其他验证都有这样有问题
		tag.nextAll('.error').html(msg);
		tag.parent().addClass("bomp-error");
	}else{
		tag.nextAll('.error').html("");
		tag.parent().removeClass("bomp-error");
	}
