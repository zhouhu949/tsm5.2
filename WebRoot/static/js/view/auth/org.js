...
$(function () {
    $("#data-list").datagrid({
        ...
        columns: [[
            ...
            {field: 'orgName', title: '名称', width:100, align: 'center'},
          //  {field: 'orgEmail', title: '邮箱', width: 50, align: 'center'},
         //   {field: 'linkName', title: '联系人', width: 50, align: 'center'},
            {field: 'createTime', title: '录入时间', width: 80, align: 'center', sortable: true},
            {
                field: 'opt2', title: '操作', width: 50, align: 'center',
                formatter: function (value, row, index) {
                    var str="";
                   // str+="<a class='edit_' href=javascript:openDelete('" + row.orgId + "') style='cursor:hand'>删除</a>";
                   // str+="&nbsp;&nbsp;<a class='edit_' href=javascript:openInfo('" + row.orgId + "') style='cursor:hand'>编辑</a>";
                    str+="&nbsp;&nbsp;<a class='edit_' href=javascript:openInfo('" + row.orgId + "') style='cursor:hand'>详情</a>";
                    return str;
                }
            }
        ]],
...
function openInfo() {
   ...
   // alert(selection.resourceId);
    ...
    parent.addTabFun({
        src:"/org/edit?orgId=" + selection.orgId,
        title: "修改机构"
    });
}
function openDelete() {
    ...
    $.messager.confirm("提示", "确定要删除【" + selection.orgName + "】吗？", function (r) {
        if (r) {
          //  $.messager.alert('提示', r?"是":"否", 'error');
            $.ajax({
             type: 'POST',
             url: ctx + "/org/delete",
             data: {orgid: selection.orgId},
             dataType: 'text',
