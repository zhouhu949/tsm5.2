...
function custsLayout(groupIds){
	if($('#main01-nodata').length > 0){
		...
		$.ajax({
			url : ctx + '/layout/user/custStateChart?type=1',
			type : 'post',
...
//				var poolNum = data.poolNum;
				...
				if($(".hyx-silent-p").length > 0){
                    myChart.on("click",function(param){
						...
						window.parent.$("#iframepage").attr("src",ctx+"/layout/team/custStatusReport?groupIds="+groupIds+"&statusStr="+param.data.status);
					});
				}
				$("#allCustNum").text((custNum+signNum+silentNum+losingNum+resNum)+"个");
				$("#intCustNum").text(custNum+"个");
...
function saleProcLayout(groupIds){
	if($('#main02-nodata').length > 0){
		...
		$.ajax({
			url:ctx+'/layout/user/saleProcChart?type=1',
			type:'post',
			data:{groupIds:groupIds},
			dataType:'json',
			error:function(){},
			success:function(data){
				...
				if($(".hyx-silent-p").length > 0){
                    myChart2.on("click",function(param){
						...
						window.parent.$("#iframepage").attr("src",ctx+"/layout/team/saleProcReport?groupIds="+groupIds+"&optionlistIds="+param.data.optionlistId);
					});
				}
...
function custTypeLayout(groupIds){
	if($('#main03-nodata').length > 0){
		...
		$.ajax({
			url:ctx+'/layout/user/custTypeChart?type=1',
			type:'post',
			data:{groupIds:groupIds},
			dataType:'json',
			error:function(){},
			success:function(data){
				...
				if($(".hyx-silent-p").length > 0){
                    myChart3.on("click",function(param){
						...
						window.parent.$("#iframepage").attr("src",ctx+"/layout/team/custTypeReport?groupIds="+groupIds+"&optionlistIds="+param.data.optionlistId);
					});
				}
				var totalNum = data.totalNum;
				var html = '<li class="clearfix">'
...
function productLayout(groupIds){
	if($('#main04-nodata').length > 0){
		...
		$.ajax({
			url:ctx+'/layout/user/productChart?type=1',
			type:'post',
			data:{groupIds:groupIds},
			dataType:'json',
			error:function(){},
			success:function(data){
				...
				if($(".hyx-silent-p").length > 0){
						...
						window.parent.$("#iframepage").attr("src",ctx+"/layout/team/productReport?groupIds="+groupIds+"&productIds="+param.data.productId);
					});
				}
				var totalNum = data.totalNum;
				var html = '<li class="clearfix">'
