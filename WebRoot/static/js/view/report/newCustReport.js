$(function() {
			...
			success:function(data){
				if(data.status){
					$("#addReportForm").ajaxSubmit({
						url:ctx+"/custom/report/addCustomReport",
						success:function(data){
							if(data.status){
	...
	$("body").delegate(".chooseType","change",function(){
		...
		$this.parents(".step-top-operation-box-item").find("select>option").show();
		$this.parents(".step-top-operation-box-item").find("select").each(function(i,obj){
			var _this = $(obj);
