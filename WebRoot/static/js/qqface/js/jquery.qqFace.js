	...
	$.fn.qqFace = function(options){
		...
		$(this).click(function(e){
			...
			if($('#'+id).length<=0){
				...
				for(var i=1; i<=59; i++){
					labFace = '['+tip+i+']';
					strFace += '<td><img width="24px" height="24px" src="'+path+i+'.gif" onclick="$(\'#'+option.assign+'\').setCaret();$(\'#'+option.assign+'\').insertAtCaret(\'' + labFace + '\');" /></td>';
...
jQuery.fn.extend({ 
	...
	insertAtCaret: function(textFeildValue){ 
		...
		function replace_em(str){
			...
			str = str.replace(/\>/g,'&gt;');
			str = str.replace(/\n/g,'<br/>');
			str = str.replace(/\[em_([0-9]*)\]/g,'<img width="30px" height="30px" src='+ctx+'"/static/js/qqface/arclist/$1.gif" border="0" />');
			return str;
		}
