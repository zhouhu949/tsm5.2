...
$.fn.extend({
    sorttable: function (setting) {
        ...
        // 可排序行的头列（jquery对象——集合）
        var headCells = extObj.find("tr[role='head']>th>[" + myAttrs.sort + "='true']");
        headCells.each(function () {
        	if (configer.ascImgUrl != "" && configer.descImgUrl != "" && configer.unSortImgUrl != "") {
            	$("<img class='unsort' src='" + configer.unSortImgUrl + "' style='width:" + configer.unSortImgSize + "; height:" + configer.ascImgSize + ";margin-left:5px;'/>").appendTo($(this));
                $("<img class='asc' src='" + configer.ascImgUrl + "' style='width:" + configer.ascImgSize + "; height:" + configer.ascImgSize + ";display:none;margin-left:5px;' title='升序' alt='升序'/>").appendTo($(this));
                $("<img class='desc' src='" + configer.descImgUrl + "' style='width:" + configer.descImgSize + "; height:" + configer.descImgSize + ";display:none;margin-left:5px;' title='降序' alt='降序'/>").appendTo($(this));
            }
            else {
                $("<span class='asc' style='width:12px; height:12px;display:none;margin-left:5px;' title='升序'>&#118;</span>").appendTo($(this));
                $("<span class='desc' style='width:12px; height:12px;display:none;margin-left:5px;' title='降序'>&#94;</span>").appendTo($(this));
            }
        ...
        function SetColumnOrder(headCell) {
            ...
            var rs = extObj.find("tr[role!='head']").clone(true);
            var isHideSearch = headCell.attr("hidesort");
            rs.sort(function (r1, r2) {
            	var a,b;
            	if(isHideSearch == "true"){
            		a= $.trim($(r1).children("td").eq(index).attr("hidevalue"));
                    b = $.trim($(r2).children("td").eq(index).attr("hidevalue"));
            	}else{
	                 a= $.trim($(r1).children("td").eq(index).text());
	                 b = $.trim($(r2).children("td").eq(index).text());
            	}
            	if(a == "" && b != ""){
            		return 1;
            	}
...
//            	}
            ...
            headCells.children(".unsort").show();
            headCell.find("span:first").children(".unsort").hide();
            if (by == sortOrder.byAsc) {
                headCell.find("span:first").children(".asc").show();
            }
            else {
                headCell.find("span:first").children(".desc").show();
            }
        }
