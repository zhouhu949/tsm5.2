...
~function($, window, undefined) {
    ...
    dialog = function (options) {
        if (options.id && dialog.get[options.id] !== undefined && $("#" + options.id).html() != undefined) {
            // alert(dialog.get[options.id]+"*****"+options.id);
            /* if($("#"+options.id).html()==undefined){
             new dialog.fn.init(options);
             }*/
            // alert("alert===="+($("#"+options.id).html()==undefined));
            //  new  dialog.fn.init(options);
    ...
    dialog.fn = dialog.prototype = {
        init: function(options) {
            ...
            this.background = options.background || '#305271';
        ...
        createHTML: function() {
            ...
            var dialog = document.createElement('div'),
                html = '',
                that = this;
            if (this.id != undefined) $(dialog).attr("id", this.id);
            if (this.title!==false) {
